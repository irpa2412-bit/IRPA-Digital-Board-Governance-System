...
import {createMemberRegistration,getRecords,updateRecord,deleteRecord,synchronizeRegisteredIdentityUids,COLLECTIONS}from"../firebase/data";
...
export default function Members(){
 ...
 return <div className="page">
  <div className="page-header"><div><span className="eyebrow">MEMBERSHIP REGISTER</span><h1>Members Registration</h1><p>Independent IRPA Member register. Members receive IRPA-MEM registration numbers. Board Members are registered separately in the Board Members platform.</p></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button type="button" onClick={syncUids} disabled={uidBusy||busy}>{uidBusy?"Synchronizing UIDs…":"Synchronize Firebase UIDs"}</button><button type="button" onClick={()=>{reset();setError("");setMessage("")}}>+ Register Member</button></div></div>
  ...
  <section className="panel"><div className="panel-header"><div><h2>Official Member Register</h2><span>{filtered.length} of {records.length} registered Member(s)</span></div><input className="table-search"placeholder="Search Members..."value={search}onChange={e=>setSearch(e.target.value)}/></div>
   <div className="table-wrapper"><table><thead><tr><th>Member Number</th><th>Name</th><th>Member Type</th><th>Email</th><th>UID Status</th><th>Status</th><th>Actions</th></tr></thead><tbody>{filtered.length===0?<tr><td colSpan="7">No Members registered.</td></tr>:filtered.map(r=><tr key={r.id}><td><strong>{r.memberNumber}</strong></td><td>{r.name||"—"}</td><td>{r.memberType||"—"}</td><td>{r.email||"—"}</td><td><span className="status-badge">{r.uid?`UID Linked · ${String(r.uid).slice(0,8)}…`:r.uidAssignmentStatus||"UID Pending"}</span></td><td><span className="status-badge">{r.status||"—"}</span></td><td><div className="member-actions"><button type="button"onClick={()=>setViewing(r)}>View</button><button type="button"onClick={()=>edit(r)}>Edit</button><button type="button"className="danger-button"onClick={()=>remove(r)}>Delete</button></div></td></tr>)}</tbody></table></div>
  </section>
  {viewing&&<section className="panel"><div className="panel-header"><div><h2>Member Profile</h2><span>{viewing.memberNumber}</span></div><button className="secondary-button"onClick={()=>setViewing(null)}>Close</button></div><div className="detail-grid">{[["Member Number",viewing.memberNumber],["Firebase UID",viewing.uid],["UID Status",viewing.uidAssignmentStatus],["Full Name",viewing.name],["Official Email",viewing.email],["Member Type",viewing.memberType],["Phone",viewing.phone],["Gender",viewing.gender],["Nationality",viewing.nationality],["Address",viewing.address],["Status",viewing.status],["Registration Status",viewing.registrationStatus],["Biography",viewing.biography]].map(([a,b])=><div key={a}><span>{a}</span><strong>{b||"—"}</strong></div>)}</div></section>}
 </div>;
}