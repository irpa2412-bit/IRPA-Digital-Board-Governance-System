...
export default function InductionOrientation(){
 ...
 const roleOptions=useMemo(()=>unique([...(context?.roles||[]),...(context?.invitations||[]).flatMap(x=>[x.role,...(x.roles||[])]),...registerEmployees.flatMap(x=>[x.role,...(x.roles||[])]),...registerMembers.flatMap(x=>[x.role,...(x.roles||[])]),form.primaryRole]).filter(Boolean),[context,registerEmployees,registerMembers,form.primaryRole]);
...
// A successful server receipt always clears the applicant entry form.
// Routing outcome is displayed separately and cannot prevent clearing.
   clearSubmittedForm();if(result.routingStatus==="Advanced to Administrator"){setMessage(`APPLICATION SUBMITTED — ADVANCED TO ADMINISTRATOR. System accuracy: ${result.accuracyPercentage}%. A decision summary is available to the Administrator. Feedback has been queued to ${submittedEmail}.`);} else if(result.routingStatus==="Filtered — Below 75% Accuracy"){setMessage(`APPLICATION SUBMITTED — BELOW THE 75% THRESHOLD. System accuracy: ${result.accuracyPercentage}%. Feedback identifying the correction items has been queued to ${submittedEmail}.`);} else setMessage("APPLICATION SUBMITTED. The application has been received by the IRPA administrator system. Feedback will be provided through the email supplied in the form.");
