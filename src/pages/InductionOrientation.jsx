 ...
 return <div className="page induction-page" aria-busy={saving?"true":"false"}>
  ...
  <section className="panel">
   ...
   <div className="detail-grid">
    <button type="button" className="detail-card interactive-detail" onClick={()=>{if(context.fullName)patch("verifiedFullName",context.fullName);document.querySelector('form')?.scrollIntoView({behavior:"smooth",block:"start"})}} style={{textAlign:"left",cursor:"pointer",width:"100%"}}><span>FULL NAME</span><strong>{context.fullName||"Applicant enters below"}</strong><small>Tap to use / edit in Applicant Entry</small></button>
    <button type="button" className="detail-card interactive-detail" onClick={()=>{if(context.accountType)patch("accountType",context.accountType);document.querySelector('form')?.scrollIntoView({behavior:"smooth",block:"start"})}} style={{textAlign:"left",cursor:"pointer",width:"100%"}}><span>REGISTERED CAPACITY</span><strong>{context.accountType||"New applicant selects below"}</strong><small>Tap to use this registered capacity</small></button>
    <button type="button" className="detail-card interactive-detail" onClick={()=>{if(context.role){const first=String(context.role).split(" • ")[0];patch("primaryRole",first)}document.querySelector('form')?.scrollIntoView({behavior:"smooth",block:"start"})}} style={{textAlign:"left",cursor:"pointer",width:"100%"}}><span>REGISTERED ROLE(S)</span><strong>{context.role||"Applicant selects below"}</strong><small>Tap to use the registered role</small></button>
    <button type="button" className="detail-card interactive-detail" onClick={()=>{if(context.department)patch("department",context.department);document.querySelector('form')?.scrollIntoView({behavior:"smooth",block:"start"})}} style={{textAlign:"left",cursor:"pointer",width:"100%"}}><span>DEPARTMENT</span><strong>{context.department||"Applicant selects below"}</strong><small>Tap to use the registered department</small></button>
    <button type="button" className="detail-card interactive-detail" onClick={()=>{if(context.unit)patch("unit",context.unit);document.querySelector('form')?.scrollIntoView({behavior:"smooth",block:"start"})}} style={{textAlign:"left",cursor:"pointer",width:"100%"}}><span>UNIT</span><strong>{context.unit||"Applicant selects below"}</strong><small>Tap to use the registered unit</small></button>
    <button type="button" className="detail-card interactive-detail" onClick={()=>patch("identityConfirmation",context.boardMember?"Yes":"Yes")} style={{textAlign:"left",cursor:"pointer",width:"100%"}}><span>BOARD MEMBER</span><strong>{context.boardMember?"Yes":"No"}</strong><small>Tap to confirm retrieved identity</small></button>
    <div><span>REGISTRATION NUMBER</span><strong>Issued by email after administrator LINK</strong></div>
    <button type="button" className="detail-card interactive-detail" onClick={()=>document.querySelector('form')?.scrollIntoView({behavior:"smooth",block:"start"})} style={{textAlign:"left",cursor:"pointer",width:"100%"}}><span>INVITATION</span><strong>{context.invitation?"Matched":isNewApplicant?"Not required for new enrollment":"No separate invitation found"}</strong><small>Tap to continue to applicant entry</small></button>
    ...
    <button type="button" className="detail-card interactive-detail" onClick={()=>document.querySelector('form')?.scrollIntoView({behavior:"smooth",block:"start"})} style={{textAlign:"left",cursor:"pointer",width:"100%"}}><span>APPLICATION FILTER</span><strong>{isNewApplicant?((registerEmployees.length||registerMembers.length)?"PASSED — READY FOR ADMINISTRATOR REVIEW":"BLOCKED — VERIFICATION REQUIRED"):"REGISTERED ACCOUNT"}</strong><small>Tap to review verification entries</small></button>
   </div>
  </section>
