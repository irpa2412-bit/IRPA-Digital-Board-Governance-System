...
if(form.identityConfirmation!=="Yes"){setError(context.anonymous?"Please confirm that the new enrollment information you entered is accurate.":"You must confirm that the displayed registration and invitation information belongs to you.");return}
  ...
  try{
   ...
   if(result.routingStatus==="Advanced to Administrator") setMessage(`Application received and advanced to the Administrator. System accuracy: ${result.accuracyPercentage}%. A short summary has been attached to the administrator review record, and feedback has been queued to ${form.verifiedEmail}. No login authorisation is issued until an administrator decision.`); else if(result.routingStatus==="Filtered — Below 75% Accuracy") setMessage(`Application received but not advanced because the system accuracy is ${result.accuracyPercentage}%, below the 75% threshold. System feedback has been queued to ${form.verifiedEmail} identifying the items requiring correction.`); else setMessage("Application received. The system is completing the administrator-routing check and will provide feedback through the email supplied in the form.");
