...
:await createSignatureEnvelope({title,documentId,documentReference:selectedDoc?.reference||selectedDoc?.title||documentId,documentUrl,documentClassification:selectedDoc?.classification||selectedDoc?.accessLevel||"Public",archiveCategory:selectedDoc?.archiveCategory||"Administrative Documents",signingMode,recipients:orderedRecipients,fields});setEnvelopes(prev=>[env,...prev]);setDraftEnvelopeId(null);setTitle("");setDocumentId("");setDocumentUrl("");setRecipients([]);setFields([]);setSelectedField(null);setTab("Envelopes");setMessage(`${env.envelopeReference} created as Draft.`)}catch(x){console.error("IRPA Signature Platform: Create Draft Envelope failed",x);setMessage(`Create Draft Envelope was not completed: ${x.message||"Unable to create envelope."}`)}finally{setBusy(false)}}
 ...
 <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
  {selected.documentUrl&&<a className="secondary-button" href={selected.documentUrl.startsWith("drive://")?"#":selected.documentUrl} target="_blank" rel="noreferrer" onClick={async e=>{if(selected.documentUrl.startsWith("drive://")){e.preventDefault();try{const bytes=await downloadDriveBytes(selected.documentUrl.slice("drive://".length));const blob=new Blob([bytes],{type:"application/pdf"});const url=URL.createObjectURL(blob);window.open(url,"_blank","noopener,noreferrer");setTimeout(()=>URL.revokeObjectURL(url),60000)}catch(x){setMessage(x.message||"Unable to open the controlled document.")}}}}>Open Controlled Document</a>}
