...
export default function SignaturePlatform({signerOnly=false,signingEnvelopeId=null,initialDocument=null}){
 ...
 useEffect(()=>{(async()=>{
    try{
      ...
      try{
        const d=await getControlledDocumentsForSigning();
        setDocs(d);
      }catch(error){
        console.warn("Controlled Documents unavailable:",error);
        setDocs([]);
        setMessage("Controlled document list could not be loaded. Check the document access authorization and try again.");
      }
      try{
...
?await finalizeSignatureWorkflowDraft({envelopeId:draftEnvelopeId,title,documentId,documentReference:selectedDoc?.reference||selectedDoc?.title||documentId,documentUrl,documentClassification:selectedDoc?.classification||selectedDoc?.accessLevel||"Public",documentArchiveCategory:selectedDoc?.archiveCategory||"Administrative Documents",signingMode,recipients:orderedRecipients,fields,ownerSigningEnabled})
:await createSignatureEnvelope({title,documentId,documentReference:selectedDoc?.reference||selectedDoc?.title||documentId,documentUrl,documentClassification:selectedDoc?.classification||selectedDoc?.accessLevel||"Public",documentArchiveCategory:selectedDoc?.archiveCategory||"Administrative Documents",signingMode,recipients:orderedRecipients,fields});setEnvelopes(prev=>[env,...prev]);setDraftEnvelopeId(null);setTitle("");setDocumentId("");setDocumentUrl("");setRecipients([]);setFields([]);setSelectedField(null);setTab("Envelopes");setMessage(`${env.envelopeReference} created as Draft.`)}catch(x){console.error("IRPA Signature Portal: Create Draft Envelope failed",x);setMessage(`Create Draft Envelope was not completed: ${x.message||"Unable to create envelope."}`)}finally{setBusy(false)}}
 async function sign(){setBusy(true);setMessage("");try{if(!selected)throw new Error("Open a signing envelope first.");const uid=auth.currentUser?.uid;if(selected.signingMode==="Sequential"&&selected.currentSignerUid!==uid)throw new Error("This envelope is waiting for another signer.");const mine=(selected.fields||[]).filter(f=>f?.signerUid===uid);if(!mine.length)throw new Error("This signing task is assigned to another officer. Your profile cannot execute another officer assignment.");if(mine.some(f=>f.required&&["Text","Name","Date","Number","Comment","Remarks"].includes(f.type)&&!fieldValues[f.fieldId]))throw new Error("Complete all required fields assigned to you.");const updated=await signEnvelope(selected,profile,fieldValues);setSelected(updated);setEnvelopes(envelopes.map(e=>e.id===updated.id?updated:e));const archived=await getMySignedDocuments();setSignedDocuments(archived);if(updated.status==="Completed"){const ownArchive=archived.find(d=>d.envelopeId===updated.id&&d.signerUid===uid&&d.signerArchiveUrl);if(!ownArchive)throw new Error("The signing ceremony completed, but the final document was not delivered to your Signature Profile archive. The transaction has not been presented as archived.");setTab("My Documents");setMessage("Signing ceremony completed. The final, fully signed document has been delivered to your Signature Profile archive and is now available in My Documents.");}else{setMessage("Your signature has been recorded. This is not yet the final signature. IRPA will track the document through the remaining signing stages; once the last required signature is applied, the final fully signed document will be delivered to your Signature Profile archive.");}}catch(x){setMessage(x.message||"Unable to complete signing.")}finally{setBusy(false)}}
 ...
 {!signerOnly&&<div style={{display:"flex",gap:8,marginBottom:18,flexWrap:"wrap"}}><div className="signature-portal-nav">{["Profile","Sign a Document","Prepare Envelope","Envelopes","Sign Document","My Documents"].map(x=><button type="button" key={x} className={tab===x?"signature-nav-button active":"signature-nav-button"} onClick={()=>setTab(x)}>{x}</button>)}</div></div>}{message&&<div className="auth-message" style={{marginBottom:16}}>{message}</div>}
 ...
 <div className="panel" style={{border:"1px solid rgba(109,93,252,.4)",background:"rgba(109,93,252,.06)"}}>
   <div className="form-grid">
     ...
     <label>Controlled document<div style={{display:"flex",gap:6,alignItems:"stretch"}}><select value={initiateDocumentId} onChange={e=>setInitiateDocumentId(e.target.value)} disabled={busy} aria-label="Controlled document">{docs.length===0?<option value="">No controlled PDF documents available</option>:<option value="">Select controlled document</option>}{docs.map(d=><option key={d.id} value={d.id}>{d.reference||d.title||d.fileName||d.id}</option>)}</select><button type="button" className="text-button" onClick={async()=>{if(busy)return;setBusy(true);setMessage("Refreshing controlled documents…");try{setDocs(await getControlledDocumentsForSigning());setMessage("Controlled document list refreshed.");}catch(error){setMessage("Unable to refresh controlled documents: "+(error?.message||"access denied."));}finally{setBusy(false)}}} disabled={busy} title="Refresh controlled documents" aria-label="Refresh controlled documents">↻</button></div></label>
 ...
 </div>}<button type="submit" className="signature-profile-primary-button" style={{marginTop:18}} disabled={busy||(!signatureFile&&!drawnSignature)}>{busy?"Saving...":profile?"Save Signature Profile":"Save Signature Profile"}</button>{profile&&<button type="button" className="signature-profile-secondary-button" style={{marginTop:12,border:replacementMode?"2px solid #f87171":"1px solid rgba(255,255,255,.2)",fontWeight:800}} disabled={busy} onClick={()=>{setReplacementMode(true);setSignatureFile(null);setDrawnSignature("");setMessage("REPLACEMENT MODE ACTIVE: draw the new handwritten signature in the box above, then press Save Signature. The current served signature remains active until the save succeeds.");document.querySelector("[data-signature-specimen]")?.scrollIntoView({behavior:"smooth",block:"center"})}}>{replacementMode?"Replacement Mode Active":"Replace Signature"}</button>}<div className="auth-message" role="status" aria-live="polite" style={{marginTop:12}}>{busy?"Saving signature to your persistent Signature Profile…":replacementMode?"REPLACEMENT MODE ACTIVE: draw the new signature and press Save Signature. The current served signature remains unchanged until the save succeeds.":profile?"CURRENT SIGNATURE SERVED: this signature will be reused for future documents.":"NO SIGNATURE SERVED: save your handwritten specimen above to activate it."}</div></form></section>}
 {!signerOnly&&tab==="My Documents"&&<section className="panel">
 <div className="panel-heading"><div><span className="eyebrow">SIGNATURE PROFILE ARCHIVE</span><h2>My Documents</h2><p className="muted">Every document you personally sign is tracked as part of its signing ceremony. If another required signer remains, the document stays in workflow tracking. After the last required signature is applied, the final fully signed document is delivered to your Signature Profile archive, regardless of who initiated the workflow.</p></div></div>
 ...
 {signedDocuments.length===0?<p className="muted">No signed documents are recorded for this Signature Profile yet.</p>:<div style={{display:"grid",gap:10}}>{signedDocuments.map(d=><article key={d.id} className="stat-card"><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"flex-start"}}><div><span>{d.documentReference||d.documentId||"Signed Document"}</span><h3 style={{margin:"5px 0"}}>{d.documentTitle||d.title||d.documentReference||"Signed Document"}</h3><small>Signed by you · {d.workflowRole||"Signature action"} · {d.signedAt?.seconds?new Date(Number(d.signedAt.seconds)*1000).toLocaleString():d.signedAt?new Date(d.signedAt).toLocaleString():"Date recorded"}</small><div style={{fontSize:11,opacity:.65,marginTop:5,wordBreak:"break-all"}}>SHA-256: {d.signedDocumentHash||"Recorded"}</div><div style={{fontSize:11,marginTop:5,opacity:.75}}>My Documents Portal: {d.myDocumentsPortalPath||"Completed Signed Documents"}</div></div><button type="button" className="secondary-button" onClick={async()=>{try{const target=d.signerArchiveUrl||"";if(!target)throw new Error("The archived signed PDF link is missing.");if(target.startsWith("drive://")){const bytes=await downloadDriveBytes(target.slice("drive://".length));const blob=new Blob([bytes],{type:"application/pdf"});const url=URL.createObjectURL(blob);window.open(url,"_blank","noopener,noreferrer");setTimeout(()=>URL.revokeObjectURL(url),60000)}else window.open(target,"_blank","noopener,noreferrer")}catch(error){setMessage(error.message||"Unable to open the archived signed document.")}}}>Open Signed PDF</button></div></article>)}</div>}
 </section>}
 {!signerOnly&&tab==="Prepare Envelope"&&<section className="panel"><div className="panel-heading"><div><span className="eyebrow">ENVELOPE PREPARATION</span><h2>Prepare, Assign & Route Document</h2><p className="muted">Upload/select the controlled PDF, place comments, numbers, remarks, dates and signatures, assign every field to an officer, and route the document in the required sequence.</p></div></div><form onSubmit={createEnvelope} noValidate><div className="form-grid"><label>Envelope title<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Board Resolution Approval"/></label><label>Controlled document<select value={documentId} onChange={e=>setDocumentId(e.target.value)} disabled={busy} aria-label="Controlled document">{docs.length===0?<option value="">No controlled PDF documents available</option>:<option value="">Select controlled document</option>}{docs.map(d=><option key={d.id} value={d.id}>{d.reference||d.title||d.fileName||d.id}</option>)}</select></label><label>Signing route<select value={signingMode} onChange={e=>setSigningMode(e.target.value)}><option>Sequential</option><option>Parallel</option><option>Mixed</option></select></label></div>
