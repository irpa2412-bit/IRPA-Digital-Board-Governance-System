...
export default function IRPADBGSMeetingRoomMedia({meeting,selectedAuthority="",controller=false}){
 ...
 async function connect(){
   ...
   try{
     ...
     console.error("IRPA-DBGS Meeting Room media connection failed",e);
     setStatus("READY");
     setError(e?.message||"Unable to connect to the IRPA-DBGS Meeting Room.");
   }
 }
 ...
 return <section className="panel" style={{marginTop:18}}>
   <div className="panel-header">
     <div><span className="eyebrow">IRPA-DBGS MEETING ROOM</span><h2>Secure Live Meeting</h2><p className="panel-description">Governance control remains in IRPA-DBGS. Live audio, video, data and screen sharing are provided by the secured meeting-media layer.</p></div>
   ...
   <div className="form-actions" style={{marginTop:14}}>
     {status!=="CONNECTED"&&<button onClick={connect} disabled={!controller||status==="AUTHORIZING"}>{status==="AUTHORIZING"?"Authorizing…":"Join IRPA-DBGS Meeting Room"}</button>}
