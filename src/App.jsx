...
function Dashboard({admin,employee,profile,onNavigate}){
 ...
 const displayName=profile?.name||employee?.name||employee?.fullName||profile?.email||"IRPA User";
 ...
 const photo=profile?.photoUrl||profile?.photoURL||profile?.avatarUrl||profile?.profilePhotoUrl||employee?.photoUrl||employee?.photoURL||employee?.avatarUrl||"";
 ...
 const cards=admin?[["Employees",stats.employees,"Current register","Members & Personnel"],["Members",stats.members,"Governance register","Members & Personnel"],["Meetings",stats.meetings,"This month","Meetings"],["Pending Authorizations",stats.authorizations,"Awaiting action","Authorization & Approvals"]]:isMember?[["My Meetings",stats.meetings,"Meeting register","Meetings"],["My Actions",stats.actions,"Implementation control","Actions"],["Pending Authorizations",stats.authorizations,"Awaiting action","Authorization & Approvals"],["Pending Signatures",stats.signatures,"Signature platform","Signature Platform"]]:[["My Meetings",stats.meetings,"Meeting register","Meetings"],["Actions Assigned",stats.actions,"Implementation control","Actions"],["Pending Authorizations",stats.authorizations,"Awaiting action","Authorization & Approvals"],["My Payment Requests",stats.payments,"Payment workflow","Employee Payments"]];
 const quick=admin?[["People & Membership","Members & Personnel","Manage members, staff and committees","groups"],["Meetings","Meetings","Schedule, manage and record meetings","calendar"],["Documents","Documents","Access authorised governance documents","document"],["Authorizations","Authorization & Approvals","Approve and track pending items","check"],["Finance","Finance Portfolio","Budgets, transactions and financial reports","finance"],["Reports","Reports","View reports and governance analytics","analytics"],["Risk Register","Risk Register","Identify and manage institutional risks","risk"],["Projects","Reports","View programme and project evidence","analytics"],["Settings","Settings","Manage your personal workspace","settings"]]:[["Meetings","Meetings","Schedule and manage assigned meetings","calendar"],["Documents","Documents","Access authorised governance documents","document"],["Finance","Finance Portfolio","Access authorised finance workflows","finance"],["Authorizations","Authorization & Approvals","Review assigned approvals","check"],["Signatures","Signature Platform","Complete assigned signatures","signature"],["Reports","Reports","View authorised reports","analytics"]];
 const iconFor=label=>({Employees:"👥",Members:"👥",Meetings:"▣","Pending Authorizations":"✓","My Actions":"☷","Actions Assigned":"☷","My Payment Requests":"¤","My Meetings":"▣","Pending Signatures":"✎","People:" :""}[label]||({groups:"👥",calendar:"▣",document:"▤",check:"✓",finance:"¤",analytics:"▥",risk:"⚠",settings:"⚙",signature:"✎"}[label]||"•"));
 ...
 return <div className="page dashboard-command">
   <div className="dashboard-desktop-view">
    ...
    <section className="dashboard-quote4-main">
      <div className="dashboard-quote4-activity panel">
       ...
       <div className="quote4-activity-table">{activity.length?activity.map((x,i)=><button type="button" key={x.id||i} onClick={()=>go(x._kind==="Authorization"?"Authorization & Approvals":x._kind==="Meeting"?"Meetings":x._kind==="Payment"?"Employee Payments":"Actions")}><span>{x.date||x.updatedAt||x.createdAt||"—"}</span><strong>{activityLabel(x)}</strong><small>{x.referenceNumber||x.reference||x.employeeNumber||x._kind}</small><em>{activityStatus(x)}</em><b>›</b></button>):<div className="quote4-empty">Live governance activities will appear here as records are created.</div>}</div>
      </div>
   ...
   <div className="dashboard-mobile-view" aria-label="Mobile IRPA governance dashboard">
    <section className="mobile-quote1-hero">
      ...
      <div className="mobile-quote1-top"><img src="/irpa-logo-pitch.svg" alt="IRPA"/><div><strong>IMPROVEMENT OF RANGELAND</strong><small>IN PASTORAL AREAS</small></div><button type="button" onClick={()=>go("Alerts")} aria-label="Notifications">●</button></div>
    ...
    <section className="mobile-quote1-quick"><div className="mobile-quote1-heading"><h2>Quick Access</h2><button type="button" onClick={()=>go("Meetings")}>View all ›</button></div><div className="mobile-quote1-grid">{quick.slice(0,9).map(([label,target,desc,ico])=><button key={target} type="button" onClick={()=>go(target)}><span>{iconFor(ico)}</span><strong>{label}</strong></button>)}</div></section>
    <section className="mobile-quote1-motto"><span>IRPA</span><strong>Sustainable rangelands · resilient communities</strong></section>
   </div>
 </div>
