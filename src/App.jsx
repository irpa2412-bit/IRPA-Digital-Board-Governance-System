...
function resolveLoginCategories(profile,employee,admin){if(admin)return["ADMINISTRATOR"];const values=[...(Array.isArray(profile?.roles)?profile.roles:[]),...(Array.isArray(profile?.assignedRoles)?profile.assignedRoles:[]),...(Array.isArray(profile?.selectedRoles)?profile.selectedRoles:[]),...(Array.isArray(profile?.roleAssignments)?profile.roleAssignments:[]),profile?.role,profile?.boardPosition,employee?.role,employee?.boardPosition,...(Array.isArray(employee?.roles)?employee.roles:[]),...(Array.isArray(employee?.assignedRoles)?employee.assignedRoles:[]),...(Array.isArray(employee?.selectedRoles)?employee.selectedRoles:[]),...(Array.isArray(employee?.roleAssignments)?employee.roleAssignments:[])].map(v=>String(v||"").trim()).filter(Boolean);return [...new Set(values)];}
