...
function isITRole(value){const role=String(value||"").trim().toLowerCase();return role==="it"||role==="it officer"||role==="information technology"||role.startsWith("it ")||role.startsWith("information technology");}
function resolveLoginCategories(profile,employee,admin){if(admin)return["ADMINISTRATOR"];const administratorAvailable=profile?.administratorAvailable===true||profile?.authorizationType==="administrator_candidate";const boardPosition=String(profile?.boardPosition||employee?.boardPosition||"").trim();const boardRoles=["Board Chairperson","Board Vice Chairperson","Board Secretary","Board Treasurer","Board Member"];const normalize=v=>String(v||"").trim();const values=[...(Array.isArray(profile?.roles)?profile.roles:[]),...(Array.isArray(profile?.assignedRoles)?profile.assignedRoles:[]),...(Array.isArray(profile?.selectedRoles)?profile.selectedRoles:[]),...(Array.isArray(profile?.roleAssignments)?profile.roleAssignments:[]),profile?.role,employee?.role,...(Array.isArray(employee?.roles)?employee.roles:[]),...(Array.isArray(employee?.assignedRoles)?employee.assignedRoles:[]),...(Array.isArray(employee?.selectedRoles)?employee.selectedRoles:[]),...(Array.isArray(employee?.roleAssignments)?employee.roleAssignments:[])].map(normalize).filter(Boolean);const filtered=values.filter(v=>!(boardPosition&&boardRoles.includes(v)&&v!==boardPosition));if(boardPosition)filtered.unshift(boardPosition);if(administratorAvailable)filtered.unshift("Administrator");return [...new Set(filtered)];}
...
export default function App(){
  ...
  const itGatewayMode=params.get("itGateway")==="1"||entryRoute==="it";
...
useEffect(()=>{
  ...
  const unsubscribe=observeAuthState(async u=>{
    ...
    try{
      ...
      if(itGatewayMode){
        const itEmployee=await getCurrentEmployeeProfile().catch(()=>null);
        const itActive=Boolean(itEmployee)&&(["active","activated"].includes(String(itEmployee?.status||"").trim().toLowerCase())||String(itEmployee?.employmentStatus||"").trim().toLowerCase()==="active"||String(itEmployee?.registrationStatus||"").trim().toLowerCase()==="activated");
        const itRoles=roleValues(itEmployee?.roles||itEmployee?.assignedRoles||itEmployee?.selectedRoles||itEmployee?.roleAssignments||itEmployee?.role);
        if(!itActive||!itRoles.some(isITRole)){setError("This IT Gateway requires an active IRPA Employee record with a registered IT role.");setProfile(null);return;}
        const itRole=itRoles.find(isITRole)||"IT";
        try{window.sessionStorage.setItem("irpaActiveAccessAuthority",itRole);window.sessionStorage.setItem("irpaAccessAuthoritySelectionState","GATEWAY_DECLARED");window.sessionStorage.removeItem("irpaPendingAccessAuthority")}catch{}
        setEmployee(itEmployee);setProfile({...itEmployee,uid:u.uid,email:u.email||itEmployee?.email||"",role:itRole,roles:[itRole],administratorAvailable:false,authorizationType:"member",enrollmentType:"employee"});
        window.history.replaceState({},document.title,window.location.pathname);
        return;
      }
      ...
      if(adminDirect?.active===true){
        ...
        const adminMemberAuthorized=Boolean(adminMember)&&(
          ["active","activated"].includes(String(adminMember?.status||"").trim().toLowerCase())||
          String(adminMember?.registrationStatus||"").trim().toLowerCase()==="activated"
        );
        const adminEmployeeAuthorized=Boolean(adminEmployee)&&(
          ["active","activated"].includes(String(adminEmployee?.status||"").trim().toLowerCase())||
          String(adminEmployee?.employmentStatus||"").trim().toLowerCase()==="active"||
          String(adminEmployee?.registrationStatus||"").trim().toLowerCase()==="activated"
        );
