...
function LiveWeatherPanel(){
 ...
 const latitude=-2.73319,longitude=36.69773;
 const loadWeather=async()=>{
  if(!apiKey){setError("Google Weather is not configured");setLoading(false);return;}
  try{
   setError("");
   const params=new URLSearchParams({key:apiKey,"location.latitude":String(latitude),"location.longitude":String(longitude),unitsSystem:"METRIC"});
 ...
 return <div className="live-weather-content" aria-live="polite">
  <div className="live-weather-current">
   <div className="live-weather-current-main"><span className="live-weather-icon" aria-hidden="true">{icon(current?.weatherCondition?.type)}</span><div><strong>{Number.isFinite(temp)?Math.round(temp)+"°C":"Live weather"}</strong><span>{description}</span><small>Longido, Tanzania · feels like {Number.isFinite(feels)?Math.round(feels)+"°C":"—"}</small></div></div>
