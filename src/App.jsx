...
function Downloads(){const[installEvent,setInstallEvent]=useState(null);const[busy,setBusy]=useState("");const[message,setMessage]=useState("");useEffect(()=>{const handler=e=>{e.preventDefault();setInstallEvent(e)};window.addEventListener("beforeinstallprompt",handler);return()=>window.removeEventListener("beforeinstallprompt",handler)},[]);function feedback(text){setMessage(text)}async function install(){setMessage("");setBusy("install");try{if(installEvent){await installEvent.prompt();const choice=await installEvent.userChoice;feedback(choice?.outcome==="accepted"?"Installation completed: IRPA Digital Governance was added to your PC.":"Installation was cancelled.");setInstallEvent(null);return}feedback("Automatic installation is not available in this browser. Open the browser menu and choose “Install this site as an app”.")}catch(e){feedback("Installation could not be completed: "+(e?.message||"Please use the browser install menu."))}finally{setBusy("")}}function shortcut(){setMessage("");setBusy("shortcut");try{const blob=new Blob(["[InternetShortcut]\\r\\nURL="+window.location.origin+"\\r\\nIconIndex=0\\r\\n"],{type:"application/internet-shortcut"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="IRPA-Digital-Governance.url";document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);feedback("PC shortcut download started. Check your browser Downloads folder.");}catch(e){feedback("PC shortcut download failed: "+(e?.message||"Unknown error."))}finally{setBusy("")}}function openDownload(url,label){setMessage("");setBusy(label);try{window.open(url,"_blank","noopener,noreferrer");feedback(label+" opened in a new browser tab. If the download does not start, check the new tab or browser Downloads.");}catch(e){feedback(label+" could not be opened: "+(e?.message||"Unknown error."))}finally{setBusy("")}}return <div className="page"><section className="welcome-panel"><div><span className="eyebrow">IRPA DIGITAL GOVERNANCE</span><h1>Downloads & PC Access</h1><p>Install or download access resources for the IRPA Digital Board Governance System.</p></div><div className="identity-card"><span>Application</span><strong>Live</strong></div></section><section className="panel"><div className="panel-heading"><div><span className="eyebrow">DOWNLOAD CENTRE</span><h2>IRPA Application Downloads</h2><p className="muted">Every action below provides immediate feedback. The live application remains securely hosted online.</p></div></div><div className="dashboard-grid" style={{marginTop:18}}><div className="stat-card"><span>INSTALL ON PC</span><strong>App</strong><small>Desktop-style browser installation</small><button type="button" style={{marginTop:12}} onClick={install} disabled={!!busy}>{busy==="install"?"Opening install…":installEvent?"Install IRPA App":"Install / Add to PC"}</button></div><div className="stat-card"><span>PC SHORTCUT</span><strong>.URL</strong><small>Download a Windows Internet Shortcut</small><button type="button" className="secondary-button" style={{marginTop:12}} onClick={shortcut} disabled={!!busy}>{busy==="shortcut"?"Preparing download…":"Download PC Shortcut"}</button></div><div className="stat-card"><span>ANDROID APP</span><strong>.APK</strong><small>Open the Android build/download page</small><button type="button" className="secondary-button" style={{marginTop:12}} onClick={()=>openDownload("https://github.com/irpa2412-bit/IRPA-Digital-Board-Governance-System/actions/runs/35520435278","Android APK download page")} disabled={!!busy}>{busy==="Android APK download page"?"Opening…":"Download Android APK"}</button></div><div className="stat-card"><span>SOURCE PACKAGE</span><strong>.ZIP</strong><small>Download the current GitHub source package</small><button type="button" className="secondary-button" style={{marginTop:12}} onClick={()=>openDownload("https://github.com/irpa2412-bit/IRPA-Digital-Board-Governance-System/archive/refs/heads/main.zip","Source ZIP download")} disabled={!!busy}>{busy==="Source ZIP download"?"Opening…":"Download Source ZIP"}</button></div></div>{message&&<div className="auth-message" style={{marginTop:18}} role="status" aria-live="polite">{message}</div>}<div className="form-actions" style={{marginTop:18}}><button type="button" onClick={()=>{setMessage("Opening the live IRPA application…");window.open(window.location.origin,"_blank","noopener,noreferrer")}}>Open Live Application</button><button type="button" className="secondary-button" onClick={()=>{setMessage("Returning to the IRPA application…");window.location.href=window.location.origin}}>Return to Application</button></div></section></div>}function Shell({user,profile,admin,employee,inductionComplete,onInductionComplete}){const financePortalAccess=admin||isFinancePortalMember(profile,employee);const procurementPortalAccess=admin||isProcurementPortalMember(profile,employee);const[active,setActive]=useState(()=>{const params=new URLSearchParams(window.location.search);const requested=params.get("adminModule");const induction=params.get("induction")==="1";return admin&&requested&&ALL_MODULES.includes(requested)?requested:induction?"Induction & Orientation":"Dashboard"}),modules=admin?ALL_MODULES:(inductionComplete?memberModules(profile):["Induction & Orientation"]);useEffect(()=>{const handler=()=>onInductionComplete?.();window.addEventListener("irpa:induction-completed",handler);return()=>window.removeEventListener("irpa:induction-completed",handler)},[onInductionComplete]);useEffect(()=>{if(!modules.includes(active))setActive("Dashboard")},[profile?.role,admin]);useEffect(()=>{const navigate=e=>{const detail=e?.detail;const target=typeof detail==="string"?detail:detail?.module;if(target&&modules.includes(target))setActive(target)};window.addEventListener("irpa:navigate",navigate);return()=>window.removeEventListener("irpa:navigate",navigate)},[modules]);const groups=Object.entries(NAV).map(([title,items])=>[title,items.filter(x=>modules.includes(x))]).filter(([,items])=>items.length);return <div className="app-shell"><aside className="sidebar"><div className="sidebar-brand"><div className="sidebar-brand-copy"><strong>IRPA</strong><span>Digital Governance</span><small>Improvement of Rangeland in Pastoral Areas</small></div></div><div className="authority-card"><span>{admin?"ADMINISTRATOR":"AUTHORIZED USER"}</span><strong>{profile?.role}</strong>{employee?.employeeNumber&&<small>{employee.employeeNumber}</small>}</div><nav>{groups.map(([title,items])=><div className="nav-group"key={title}><div className="nav-group-title">{title}</div>{items.map(x=><button key={x}className={active===x?"nav-item active":"nav-item"}onClick={()=>setActive(x)}><span>{x}</span>{active===x&&<i/>}</button>)}</div>)}</nav><div className="sidebar-footer"><div className="secure-label">● Secure IRPA workspace</div><button className="logout-button"onClick={logout}>Sign Out</button></div></aside><section className="main-area"><header className="topbar"><div><span className="topbar-kicker">IMPROVEMENT OF RANGELAND IN PASTORAL AREAS</span><h2>{active}</h2><p>Digital Board Governance Workspace</p></div><div className="user-info"><div className="user-avatar">{(profile?.name||user.email||"I").slice(0,1).toUpperCase()}</div><div><strong>{profile?.name||"IRPA User"}</strong><span>{profile?.role||user.email}</span></div></div></header><main className="content-area"><ModuleInterlinkBar active={active} onNavigate={setActive} admin={admin} role={profile?.role}/>{active==="Dashboard"&&<Dashboard admin={admin}employee={employee}/>} {active==="Board Members Registration"&&<BoardMembers/>} {active==="Invitations"&&<Invitations/>} {active==="Members & Personnel"&&<Employees/>} {active==="Meetings"&&<Meetings onNavigate={setActive}/>} {active==="Resolutions"&&<Resolutions/>} {active==="Voting"&&<Voting/>} {active==="Participants"&&<OperationalGatewaysParticipants/>} {active==="Authorization & Approvals"&&<AuthorizationApprovals/>} {active==="Signature Platform"&&<SignaturePlatformWithUpload/>} {active==="Induction & Orientation"&&<InductionOrientation profile={profile} employee={employee}/>} {active==="Downloads"&&<Downloads/>} {active==="Finance Portfolio"&&(financePortalAccess?<FinancePortfolio profile={profile} employee={employee}/>:<AccessDenied user={user} reason="Finance Portal access is restricted to authorised Finance/Accounting personnel and designated approving officers."/>)} {active==="Procurement"&&(procurementPortalAccess?<ProcurementPortal profile={profile}/>:<AccessDenied user={user} reason="Procurement Portal access is restricted to authorised Procurement personnel and designated approving officers."/>)} {active==="Settings"&&<Settings admin={admin} section="settings"/>} {active==="Add Administrator"&&<Settings admin={admin} section="administrators"/>} {CONFIGURED_MODULES.includes(active)&&<OperationalGateways module={active}/>} {active==="Employee Payments"&&<EmployeePayments profile={{...profile,...employee}}/>}</main></section></div>}
class AppErrorBoundary extends React.Component{
  constructor(props){super(props);this.state={error:null}}
  static getDerivedStateFromError(error){return{error}}
  componentDidCatch(error,info){console.error("IRPA application render error",error,info)}
  render(){
    if(this.state.error){
      return <main className="auth-screen"><section className="auth-card">
        <h1>IRPA Digital Governance</h1>
        <p>Administrator authentication succeeded, but the governance workspace encountered a display error.</p>
        <p className="muted">{this.state.error?.message||"Unknown application error"}</p>
        <button onClick={()=>window.location.reload()}>Reload IRPA Workspace</button>
...
return <AppErrorBoundary><Shell user={user} profile={profile} admin={profile.authorizationType==="administrator"} employee={employee} inductionComplete={inductionComplete} onInductionComplete={()=>setInductionComplete(true)}/></AppErrorBoundary>;
}
