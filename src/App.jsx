...
function getPageNavigationItems(active,modules){
  const group=PORTAL_ROLLER_GROUPS.find(([parent,children])=>active===parent||children.includes(active));
  if(group){
    const items=[group[0],...group[1]].filter(item=>modules.includes(item));
    if(items.length>1)return items;
  }
  return [...new Set(modules)].filter(Boolean);
}
function PageBackNextNavigation({active,modules,onNavigate}){
  const items=getPageNavigationItems(active,modules);
...
const displayModuleName=m=>({"IT Operations":"IT Operations Portal","Dashboard":"Dashboard","Induction and Orientation":"Induction & Orientation Portal","Induction Applications":"Induction Applications Portal","Board Members Registration":"Board Members Registration Portal","Members & Personnel":"Members & Personnel Portal","Invitations":"Invitations Portal","Meetings":"Meeting Portal","Meeting Room":"Meeting Room Portal","Participants":"Participants Portal","Resolutions":"Resolutions Portal","Voting":"Voting Portal","Actions":"Actions Portal","Documents":"Documents Portal","Signature Platform":"Signature Portal","Decisions":"Decisions Portal","Risk Register":"Risk Register Portal","Authorization & Approvals":"Authorization & Approvals Portal","Employee Payments":"Employee Payments Portal","Finance Portfolio":"Finance Portal","Procurement":"Procurement Portal","Reports":"Reports Portal","Audit Trail":"Audit Trail Portal","Downloads":"Downloads Portal","Settings":"Settings Portal","Add Administrator":"Add Administrator Portal"}[m]||m);const roleValues=v=>Array.isArray(v)?v.flatMap(roleValues):String(v||"").split(",").map(x=>x.trim()).filter(Boolean);const hasRole=(p,roles)=>roleValues(p?.roles||p?.role).some(r=>roles.includes(r));
const NAV={IT:["IT Operations"],GOVERNANCE:["Dashboard","Induction and Orientation","Induction Applications","Board Members Registration","Members & Personnel","Invitations","Meetings","Resolutions","Voting","Actions","Documents","Signature Platform","Decisions","Risk Register","Authorization & Approvals","Employee Payments"],FINANCE:["Finance Portfolio","Procurement"],EVIDENCE:["Reports","Audit Trail"],SYSTEM:["Downloads","Settings","Add Administrator","External Auditor Portal"]};const PORTAL_CHILDREN={Meetings:["Meeting Room","Participants","Resolutions","Voting","Actions","Decisions"],"Induction and Orientation":["Induction Applications"]};const PORTAL_CHILDREN_SET=new Set(Object.values(PORTAL_CHILDREN).flat());const PORTAL_LABELS={Meetings:"Meeting Portal","Induction and Orientation":"Induction & Orientation Portal"};const ALL_MODULES=[...new Set([...Object.values(NAV).flat(),...Object.values(PORTAL_CHILDREN).flat()])];
const BASE_MEMBER_MODULES=["Dashboard","Meetings","Meeting Room","Participants","Resolutions","Voting","Actions","Documents","Signature Platform","Decisions","Authorization & Approvals","Procurement","Induction and Orientation","Downloads"];
const FINANCE_ROLES=["Finance Personnel","Finance Manager","Accountant","Finance Officer","Executive Director","Director Finance & Administration"];
const PROCUREMENT_ROLES=["Procurement Officer","Procurement Manager","Procurement Team Member"];
const PROCUREMENT_APPROVAL_ROLES=["Executive Director","Director Livestock","Livestock Director","Director Internal Oversight","Internal Oversight Director","Director Finance & Administration","Director Human Resources","Director Outreach","Director Community Development","Director Environment","Director Field","Director Operations","Operations Director","Outreach Director","Community Development Director","Environment Director","Field Director"];
const HR_ROLES=["Executive Director","Director Human Resources","HR Manager"];
const IT_ROLES=["IT Specialist","Information Technology Officer"];
const IT_OPERATIONS_MODULES=["IT Operations"];
const DEPARTMENTAL_MODULES={
  "Executive Office":["Members & Personnel","Participants","Risk Register","Reports"],
  "Internal Oversight":["Risk Register","Reports","Audit Trail","Decisions","Actions"],
  "Finance & Administration":["Finance Portfolio","Employee Payments","Reports","Audit Trail"],
  "Human Resources":["Members & Personnel","Employee Payments","Reports"],
  "Livestock":["Reports","Actions"],
  "Environment & Rangeland":["Risk Register","Reports","Actions"],
  "Outreach & Community Development":["Reports","Actions"],
  "Field Operations":["Reports","Actions"],
  "Operations":["Reports","Actions"],
  "Information Technology":["IT Operations"]
};
const ROLE_DEPARTMENT={
  "Administrator":"Executive Office","Executive Director":"Executive Office",
  "Director Internal Oversight":"Internal Oversight","Internal Oversight Director":"Internal Oversight","Internal Oversight Officer":"Internal Oversight",
  "Director Finance & Administration":"Finance & Administration","Finance Manager":"Finance & Administration","Finance Officer":"Finance & Administration","Accountant":"Finance & Administration","Finance Personnel":"Finance & Administration","Procurement Officer":"Finance & Administration","Procurement Manager":"Finance & Administration","Procurement Team Member":"Finance & Administration",
  "Director Human Resources":"Human Resources","HR Manager":"Human Resources","HR Officer":"Human Resources",
  "Director Livestock":"Livestock","Livestock Director":"Livestock","Livestock Officer":"Livestock",
  "Director Environment":"Environment & Rangeland","Environment Director":"Environment & Rangeland","Environment Officer":"Environment & Rangeland","Rangeland Officer":"Environment & Rangeland",
  "Director Outreach":"Outreach & Community Development","Outreach Director":"Outreach & Community Development","Outreach Officer":"Outreach & Community Development","Director Community Development":"Outreach & Community Development","Community Development Director":"Outreach & Community Development","Community Development Officer":"Outreach & Community Development",
  "Director Field":"Field Operations","Director Field Operations":"Field Operations","Field Director":"Field Operations","Field Assistant":"Field Operations",
  "Director Operations":"Operations","Operations Director":"Operations","Operations Manager":"Operations",
  "IT Specialist":"Information Technology","Information Technology Officer":"Information Technology"
};
const EXECUTIVE_DIRECTOR_MODULES=["Members & Personnel","Participants","Risk Register"];const CONFIGURED_MODULES=["Meeting Room","Actions","Documents","Decisions","Risk Register","Reports","Audit Trail"];const LOGIN_ROLE_OPTIONS=["Administrator","Board Chairperson","Board Vice Chairperson","Board Secretary","Board Treasurer","Board Member","Executive Director","Director Internal Oversight","Director Livestock","Director Environment","Director Finance & Administration","Director Human Resources","Director Outreach","Director Community Development","Director Field Operations","HR Manager","HR Officer","Finance Manager","Finance Officer","Accountant","Internal Oversight Officer","Rangeland Officer","Livestock Officer","Environment Officer","Outreach Officer","Community Development Officer","Programme/Technical Officer","Procurement Officer","Operations Manager","IT Specialist","Information Technology Officer","Driver","Field Assistant","Administrative Assistant","Communications Officer","Monitoring & Evaluation Officer","Project Officer","Management","Employee"];
function isFinancePortalMember(p,e){return hasRole(p,FINANCE_ROLES)||hasRole(e,FINANCE_ROLES)||(p?.department==="Finance & Administration"&&["Finance Unit","Accounting Unit"].includes(p?.unit))||(e?.department==="Finance & Administration"&&["Finance Unit","Accounting Unit"].includes(e?.unit));}
function isProcurementPortalMember(p,e){return hasRole(p,PROCUREMENT_ROLES)||hasRole(p,PROCUREMENT_APPROVAL_ROLES)||hasRole(e,PROCUREMENT_ROLES)||hasRole(e,PROCUREMENT_APPROVAL_ROLES)||(p?.department==="Finance & Administration"&&p?.unit==="Procurement Unit")||(e?.department==="Finance & Administration"&&e?.unit==="Procurement Unit");}
function memberModules(p,e,selectedAuthority){
  const selected=String(selectedAuthority||"").trim();
  if(!selected)return["Dashboard"];
  const roles=[selected];
  const m=[...BASE_MEMBER_MODULES];
  const authorityProfile={...(p||{}),roles,role:selected};
  const authorityEmployee=e?{...e,roles:[selected],role:selected}:null;
  const department=String(authorityProfile?.department||authorityEmployee?.department||ROLE_DEPARTMENT[selected]||"").trim();
  const departmental=DEPARTMENTAL_MODULES[department]||[];
  m.push(...departmental);
  // IT is a departmental extension, not a replacement for the common member gateway.
  if(IT_ROLES.includes(selected))m.push(...IT_OPERATIONS_MODULES);
  // Preserve legacy Finance/HR role recognition where an older record lacks department metadata.
  if(isFinancePortalMember(authorityProfile,authorityEmployee))m.push("Finance Portfolio","Employee Payments","Reports","Audit Trail");
  if(roles.some(r=>HR_ROLES.includes(r)))m.push("Members & Personnel","Employee Payments","Reports");
...
function PasswordEntryPopup({notice}){if(!notice?.text)return null;const tone=notice.type==="warning"?"#f59e0b":notice.type==="success"?"#22c55e":"#60a5fa";return <div className="password-entry-popup" role="status" aria-live="polite" style={{"--popup-tone":tone}}><strong>{notice.type==="warning"?"PASSWORD CAUTION":notice.type==="success"?"PASSWORD ACCEPTED":"PASSWORD ENTRY"}</strong><span>{notice.text}</span></div>}

function PasswordEntryField({label,value,onChange,minLength=6,autoComplete="current-password",notify,required=true}){const[caps,setCaps]=useState(false),[visible,setVisible]=useState(false);const checkCaps=e=>{const active=!!e?.getModifierState?.("CapsLock");setCaps(active);if(active)notify?.("Caps Lock appears to be ON. Passwords are case-sensitive; check your capital-letter setting before continuing.","warning")};const change=e=>{const next=e.target.value;onChange(next);if(next.length>=minLength&&value.length<minLength)notify?.("Password entry is complete. Review it before submitting.","info")};const blur=()=>setCaps(false);return <div style={{position:"relative"}}><label>{label}</label><div style={{position:"relative",display:"flex",alignItems:"stretch"}}><input type={visible?"text":"password"} value={value} onChange={change} onKeyDown={checkCaps} onKeyUp={checkCaps} onFocus={checkCaps} onBlur={blur} required={required} minLength={minLength} autoComplete={autoComplete} style={{paddingRight:54,width:"100%"}}/><button type="button" onClick={()=>setVisible(v=>!v)} aria-label={visible?"Hide password":"Show password"} title={visible?"Hide password":"Show password"} style={{position:"absolute",right:5,top:"50%",transform:"translateY(-50%)",width:32,height:32,minWidth:32,padding:0,border:"1px solid rgba(255,255,255,.22)",borderRadius:8,background:"rgba(255,255,255,.06)",color:"inherit",display:"inline-flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><svg width="21" height="21" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M2.5 12s3.5-5.5 9.5-5.5S21.5 12 21.5 12s-3.5 5.5-9.5 5.5S2.5 12 2.5 12Z" stroke="currentColor" strokeWidth="1.8"/><circle cx="12" cy="12" r="2.8" stroke="currentColor" strokeWidth="1.8"/></svg></button></div>{caps&&<small style={{display:"block",marginTop:5,color:"#fbbf24",fontWeight:700}}>⚠ Caps Lock appears to be ON — password case matters.</small>}</div>}
function AuthScreen(){const params=new URLSearchParams(window.location.search);const inductionMode=params.get("induction")==="1";const[mode,setMode]=useState("login"),[email,setEmail]=useState(()=>params.get("adminEmail")||""),[password,setPassword]=useState(""),[message,setMessage]=useState(""),[busy,setBusy]=useState(false),[authCooldown,setAuthCooldown]=useState(0),[passwordPopup,setPasswordPopup]=useState(null),[resetOpen,setResetOpen]=useState(false),[resetStep,setResetStep]=useState("REQUEST"),[resetConfirmation,setResetConfirmation]=useState(null),[resetPhone,setResetPhone]=useState(""),[resetCode,setResetCode]=useState("");const notifyPasswordPopup=(text,type="info")=>{setPasswordPopup({text,type});window.setTimeout(()=>setPasswordPopup(null),3600)};useEffect(()=>{if(authCooldown<=0)return;const t=window.setInterval(()=>setAuthCooldown(v=>v>0?v-1:0),1000);return()=>window.clearInterval(t)},[authCooldown]);async function verifyRole(user){
  if(inductionMode){await getCurrentInductionContext();return;}
  const withTimeout=(promise,ms,label)=>Promise.race([
    promise,
    new Promise((_,reject)=>window.setTimeout(()=>reject(new Error(label)),ms))
  ]);
  const [administrator,[member,employee]]=await Promise.all([
    withTimeout(getAdminProfile(user.uid),3000,"Administrator authorization lookup timed out.").catch(()=>null),
    Promise.all([
      withTimeout(getCurrentMemberProfile().catch(()=>null),5000,"Member authorization lookup timed out."),
      withTimeout(getCurrentEmployeeProfile({fallbackName:String(user?.displayName||"").trim()}).catch(()=>null),5000,"Employee authorization lookup timed out.")
    ])
  ]);
  const memberAuthorized=Boolean(member)&&(["active","activated"].includes(String(member?.status||"").trim().toLowerCase())||String(member?.registrationStatus||"").trim().toLowerCase()==="activated");
  const employeeAuthorized=Boolean(employee)&&(["active","activated"].includes(String(employee?.status||"").trim().toLowerCase())||String(employee?.employmentStatus||"").trim().toLowerCase()==="active"||String(employee?.registrationStatus||"").trim().toLowerCase()==="activated");
  if(administrator?.active===true||memberAuthorized||employeeAuthorized)return;
  throw new Error("Authentication succeeded, but no active IRPA authorization record was resolved for this account.");
}
async function submit(e){e.preventDefault();if(authCooldown>0){setMessage("Password sign-in is temporarily unavailable. Wait for the displayed timer, or use Forgot password? to begin verified recovery.");return}setBusy(true);setMessage("");try{if(mode==="register"){await registerWithEmail(email.trim(),password);setMessage("Account created. Please verify your email before continuing")}else{const signedIn=await loginWithEmail(email.trim(),password);await verifyRole(signedIn);if(!signedIn.multiFactor?.enrolledFactors?.length){notifyPasswordPopup("Two-way verification reminder: no second verification factor is configured for this account. Complete the security configuration after entering the IRPA workspace. Notifications remain tied to the registered email and mobile particulars.","warning");await new Promise(resolve=>window.setTimeout(resolve,2200));}else{notifyPasswordPopup("Password accepted. Secure authentication completed; opening the IRPA workspace.","success");await new Promise(resolve=>window.setTimeout(resolve,700));}}}catch(x){console.error("IRPA authentication failed:",x);setMessage(x?.message||"Authentication failed. No dashboard has been opened.");const code=String(x?.code||"");if(x?.passwordAttemptStatus==="SECOND"&&Number(x?.passwordRemainingAttempts)>0){notifyPasswordPopup("Suspension-risk warning: you have "+x.passwordRemainingAttempts+" password trial"+(x.passwordRemainingAttempts===1?"":"s")+" remaining before password access is suspended. Stop and verify your password carefully.","warning");}else if(x?.passwordAttemptStatus==="FIRST"&&Number(x?.passwordRemainingAttempts)===1){notifyPasswordPopup("Accuracy warning: one password trial remains before the 5-minute security lock. Check your password carefully before trying again.","warning");}if(code==="auth/password-locked"){setAuthCooldown(Number(x?.retryAfterSeconds||300));setMessage("Three unsuccessful password trials have been reached. Password entry is locked for 5 minutes. Do not retry until the timer expires.");}else if(code==="auth/password-suspended"){setMessage("Password access is suspended after the second set of three unsuccessful trials. Use Forgot password? and complete the registered mobile OTP and email reset process.");}else if(code==="auth/too-many-requests"){setAuthCooldown(60);setMessage("Firebase has temporarily rate-limited password sign-in. Stop retrying and use verified password recovery if needed.");}else setMessage(x.message||"Authentication failed.")}finally{setBusy(false)}}async function beginReset(){if(!email.trim()){setMessage("Enter your registered IRPA email address first.");return}setBusy(true);setMessage("");try{const result=await beginPasswordResetMobileVerification(email.trim());setResetConfirmation(result.confirmation);setResetPhone(result.maskedPhone);setResetStep("OTP");setMessage("A six-digit verification code has been sent by SMS to the registered mobile number ending "+result.maskedPhone.slice(-3)+".");}catch(x){setMessage(x.message||"Unable to start mobile password recovery.");}finally{setBusy(false)}}async function verifyReset(){if(!resetConfirmation){setMessage("Start password recovery first.");return}setBusy(true);setMessage("");try{await verifyPasswordResetMobileOtp(resetConfirmation,resetCode);await sendPasswordReset(email.trim());await clearPasswordAttemptState(email.trim());setResetStep("SENT");setMessage("Mobile verification succeeded. The password reset link has been sent to the system-registered email address. Open the newest email to set your new password.");}catch(x){setMessage(x.message||"Mobile verification failed. Check the six-digit code and try again.");}finally{setBusy(false)}}function closeReset(){setResetOpen(false);setResetStep("REQUEST");setResetConfirmation(null);setResetCode("");setResetPhone("");setMessage("")}function returnToSignIn(){window.location.href=window.location.pathname}return <main className="auth-screen"><section className="auth-card"><div className="auth-brand"><div className="brand-kicker">IRPA</div><h1>Digital Board Governance</h1><p>Secure institutional governance, authorization, finance and employee management for Improvement of Rangeland in Pastoral Areas.</p></div><div className="auth-divider"><span>{inductionMode?"Induction and Orientation":mode==="login"?"Secure sign in":"Create account"}</span></div>{inductionMode&&<div className="auth-message" style={{marginBottom:16}}><strong>Induction and Orientation — Login Assistance</strong><br/>This is a parallel assistance pathway for registered IRPA Members and Employees. If a password blocker prevents access, use <strong>Forgot password?</strong> below.</div>}<div className="auth-security-instructions"><strong>SECURITY ENTRY INSTRUCTIONS</strong><span>Use only your system-registered email address. Enter your password carefully; the eye icon at the password-field edge reveals or hides the entry. After sign-in, configure your two-way verification factor. OTP/recovery notifications are sent only to registered contact particulars.</span></div><form onSubmit={submit}><label>Email address</label><input type="email"value={email}onChange={e=>setEmail(e.target.value)}required autoComplete="email"/><PasswordEntryField label="Password" value={password} onChange={setPassword} minLength={6} autoComplete="current-password" notify={notifyPasswordPopup}/><button type="submit"disabled={busy||authCooldown>0}>{busy?"Please wait...":authCooldown>0?"Password Sign In Locked ("+Math.ceil(authCooldown/60)+" min)":mode==="register"?"Create Account":"Sign In Securely"}</button></form><div className="auth-links"><button type="button"className="secondary-button"onClick={async()=>{setBusy(true);setMessage("");try{const signedIn=await loginWithGoogle("");await verifyRole(signedIn);if(signedIn&&!signedIn.multiFactor?.enrolledFactors?.length){notifyPasswordPopup("Two-way verification reminder: no second verification factor is configured for this account. Complete the security configuration after entering the IRPA workspace.","warning");}}catch(x){setMessage(x.message||"Google sign-in failed")}finally{setBusy(false)}}}disabled={busy}>Continue with Google</button><button type="button"className="text-button"onClick={()=>setResetOpen(true)}disabled={busy}>Forgot password?</button><button type="button"className="secondary-button"onClick={()=>{window.location.href=window.location.pathname+"?induction=1&applicant=1&route=assistance"}}disabled={busy}>Induction & Orientation — Start Application</button><button type="button"className="text-button"onClick={()=>{setMode(mode==="login"?"register":"login");setMessage("")}}disabled={busy}>{mode==="login"?"Create a member account":"Return to sign in"}</button></div>{message&&<div className="auth-message">{message}</div>}<PasswordEntryPopup notice={passwordPopup}/>{resetOpen&&<div className="authority-capacity-gate" role="dialog" aria-modal="true" aria-labelledby="password-reset-title"><section className="authority-capacity-card" style={{width:"min(560px,100%)"}}><div className="authority-capacity-kicker">IRPA-DBGS · VERIFIED PASSWORD RECOVERY</div><h2 id="password-reset-title">Reset Password</h2><div id="password-reset-recaptcha" style={{minHeight:1}}></div>{resetStep==="REQUEST"&&<><p>Step 1: confirm your registered email address. The system will then send a one-time code to the registered mobile number.</p><button type="button" onClick={beginReset} disabled={busy||!email.trim()}>Send Mobile OTP</button></>}{resetStep==="OTP"&&<><p>Step 2: enter the six-digit code sent to <strong>{resetPhone}</strong>. The code must be verified before the email reset link is issued.</p><input inputMode="numeric" pattern="[0-9]*" maxLength={6} value={resetCode} onChange={e=>setResetCode(e.target.value.replace(/\D/g,"").slice(0,6))} placeholder="6-digit OTP" autoComplete="one-time-code"/><button type="button" onClick={verifyReset} disabled={busy||resetCode.length!==6}>Verify OTP &amp; Send Email Reset Link</button></>}{resetStep==="SENT"&&<><p>Two-step recovery completed: the registered mobile OTP was verified and the password reset link was sent to the system-registered email address.</p><button type="button" onClick={closeReset}>Return to Sign In</button></>}<button type="button" className="authority-capacity-signout" onClick={closeReset} disabled={busy}>Cancel</button></section></div>}</section></main>}
function Loading({message}){const[slow,setSlow]=useState(false);useEffect(()=>{const t=setTimeout(()=>setSlow(true),4000);return()=>clearTimeout(t)},[]);return <div className="loading-screen" role="status" aria-live="polite"><div><strong>IRPA Digital Governance</strong><div className="loading-sub">{message||"Authentication accepted. Verifying your registered IRPA authorization..."}</div>{slow&&!message&&<div className="loading-sub">The authorization check is taking longer than expected. The system will show the exact result instead of silently ending the session.</div>}{(slow||message)&&<button type="button" onClick={()=>window.location.reload()}>Retry Secure Session</button>}</div></div>}
...
function resolveLoginCategories(profile,employee,admin){
 const administratorAvailable=admin===true||String(profile?.authorizationType||"").toLowerCase()==="administrator";
...
function Dashboard({user,admin,employee,profile,onNavigate,selectedAuthority,authorityCategories}){
 ...
 const effectiveRole=String(selectedAuthority||profile?.role||"").trim();
 ...
 })();return()=>{live=false}},[admin,employee?.employeeNumber,profile?.role,selectedAuthority]);
 ...
 return <div className="page dashboard-command">
   ...
   <div className="dashboard-mobile-view" aria-label="Mobile IRPA governance dashboard">
    <section className="mobile-quote1-hero">
      ...
      <div className="mobile-quote1-user"><div className="mobile-quote1-photo">{photo?<img src={photo} alt={displayName}/>:<span>{initials}</span>}</div><div><span>GOOD DAY,</span><h1>{displayName}</h1><p>{profileTitle}{registrationNo?" · "+registrationNo:""}</p><em>{admin?"Administrator":selectedAuthority||profileTitle}</em></div></div>
    </section>
...
function AuthorityCapacityGate({authorities,categories,onConfirm,onSignOut}){const fallback=(Array.isArray(categories)?categories:[]).map(role=>({id:"IRPA-AUTH-"+String(role).toUpperCase().replace(/[^A-Z0-9]+/g,"-"),authorityId:"IRPA-AUTH-"+String(role).toUpperCase().replace(/[^A-Z0-9]+/g,"-"),role,pathway:"Institutional Governance Workspace",source:"Verified login context",active:true}));const normalizedAuthorities=[...new Map((Array.isArray(authorities)&&authorities.length?authorities:fallback).filter(a=>a?.active!==false&&a?.role).map(a=>[String(a.authorityId||a.id||a.role),a])).values()];const[selected,setSelected]=useState("");const[busy,setBusy]=useState(false);const[feedback,setFeedback]=useState("");const selectedAuthority=normalizedAuthorities.find(a=>String(a.authorityId||a.id)===selected);const choose=value=>{if(busy)return;const id=String(value||"");if(!normalizedAuthorities.some(a=>String(a.authorityId||a.id)===id))return;setSelected(id);setFeedback("")};const verify=async()=>{if(!selectedAuthority||busy)return;setBusy(true);setFeedback("");try{await Promise.resolve(onConfirm(selectedAuthority));}catch(error){console.error("IRPA access authority verification failed:",error);setFeedback(error?.message||"The selected access authority could not be verified. Select an available authority again.");setBusy(false)}};return <div className="authority-capacity-gate" role="dialog" aria-modal="true" aria-labelledby="authority-gate-title" aria-describedby="authority-gate-description"><section className="authority-capacity-card">{normalizedAuthorities.length===0?<><div className="authority-capacity-kicker">IRPA-DBGS · ACCESS GATEWAY</div><h1 id="authority-gate-title">Access Authority Unavailable</h1><p id="authority-gate-description">Authentication succeeded, but the server did not return a registered access authority. No internal pathway is opened.</p><div className="authority-capacity-feedback error" role="alert"><strong>ENTRY BLOCKED</strong><span>Sign out and contact the IRPA administrator to verify the institutional register.</span></div><button type="button" className="authority-capacity-signout" onClick={onSignOut}>Sign Out</button></>:<><div className="authority-capacity-kicker">IRPA-DBGS · SECURE ACCESS GATEWAY</div><h1 id="authority-gate-title">Select Your Access Authority</h1><p id="authority-gate-description">Your credentials have been authenticated. The IRPA authorization registry suggests the authority pathways already attached to this account. Select the capacity you are acting under; selection opens only the pathway already authorized for that authority.</p><div className="authority-capacity-status"><span className="authority-capacity-status-dot" aria-hidden="true"></span><div><strong>{selectedAuthority?"AUTHORITY SELECTED":"IDENTIFICATION VERIFIED · AUTHORITY PATHWAYS SUGGESTED"}</strong><small>{selectedAuthority?selectedAuthority.authorityId:"The registry has identified the authority pathways available to this authenticated identity."}</small></div></div><div className="authority-capacity-options" role="radiogroup" aria-label="Suggested authorized authority pathways">{normalizedAuthorities.map(a=>{const id=String(a.authorityId||a.id);const pathway=String(a.pathway||"Authorized pathway");const department=String(a.department||"").trim();const unit=String(a.unit||"").trim();return <button key={id} type="button" className={selected===id?"authority-capacity-option selected":"authority-capacity-option"} onClick={()=>choose(id)} aria-pressed={selected===id} role="radio" aria-checked={selected===id} disabled={busy}><span>{a.role}</span><small>{pathway}</small><small>{[department,unit].filter(Boolean).join(" · ")||id}</small></button>})}</div>{selectedAuthority&&<div className="authority-capacity-feedback" role="status"><strong>PATHWAY READY</strong><span>{selectedAuthority.authorityId} → {selectedAuthority.pathway}</span></div>}{feedback&&<div className="authority-capacity-feedback error" role="alert" aria-live="assertive"><strong>AUTHORITY VERIFICATION MESSAGE</strong><span>{feedback}</span></div>}<button type="button" className="authority-capacity-confirm" disabled={!selectedAuthority||busy} onClick={verify}>{busy?"Verifying Authority…":"Enter Authorized Pathway"}</button><button type="button" className="authority-capacity-signout" onClick={onSignOut} disabled={busy}>Sign Out</button><small className="authority-capacity-note">This gateway does not grant permissions. It selects one server-verified access authority and routes the session into the corresponding internal or departmental workspace.</small></>}</section></div>}

function Shell({user,profile,admin,employee,inductionComplete,onInductionComplete}){const authorityCategories=resolveLoginCategories(profile,employee,admin);const authorityRecords=[...(Array.isArray(profile?.authorities)?profile.authorities:[]),...authorityCategories.map(role=>({id:"IRPA-AUTH-"+String(role).toUpperCase().replace(/[^A-Z0-9]+/g,"-"),authorityId:"IRPA-AUTH-"+String(role).toUpperCase().replace(/[^A-Z0-9]+/g,"-"),role,pathway:role==="Administrator"?"Primary Administrator Workspace":"Institutional Governance Workspace",source:"Verified login context",active:true}))].filter(a=>a?.active!==false&&a?.role).reduce((acc,a)=>{const key=String(a.authorityId||a.id||a.role);if(!acc.some(x=>String(x.authorityId||x.id||x.role)===key))acc.push(a);return acc},[]);const[storedAuthority,setStoredAuthority]=useState(()=>{try{return window.sessionStorage.getItem("irpaActiveAccessAuthority")||""}catch{return""}});const selectedAuthority=authorityCategories.includes(storedAuthority)?storedAuthority:"";const sessionAdministrator=selectedAuthority==="Administrator";const setSelectedAuthority=authority=>{if(sessionAdministrator||!authorityCategories.includes(authority)||authority===selectedAuthority)return;try{window.sessionStorage.setItem("irpaPendingAccessAuthority",authority);window.sessionStorage.removeItem("irpaActiveAccessAuthority")}catch{}setStoredAuthority("");setActiveAuthorityLoginRequired(true);setMobileNav("");setActive("Dashboard");void logout();};const confirmInitialAuthority=async authority=>{if(!authority)return;const authorityId=String(authority.authorityId||authority.id||"").trim();const role=String(authority.role||"").trim();if(!authorityId||!role)throw new Error("The selected access authority is incomplete.");let verifiedAuthority=null;try{const context=await Promise.race([resolveAuthenticatedLoginContext(),new Promise((_,reject)=>window.setTimeout(()=>reject(new Error("Server access authority verification timed out.")),6000))]);const serverAuthorities=Array.isArray(context?.authorities)?context.authorities:[];verifiedAuthority=serverAuthorities.find(a=>String(a.authorityId||a.id)===authorityId&&String(a.role)===role&&a.active!==false)||null;}catch(error){console.warn("IRPA authority resolver unavailable; using the already verified authenticated identity context.",error);}
if(!verifiedAuthority){const activeStatus=[profile?.status,profile?.employmentStatus,profile?.registrationStatus].map(v=>String(v||"").trim().toLowerCase());const activeIdentity=activeStatus.some(v=>["active","activated"].includes(v))||admin?.active===true;const roleDeclared=authorityCategories.includes(role)||roleValues(profile?.roles||profile?.role).includes(role)||roleValues(employee?.roles||employee?.role).includes(role)||role==="Administrator";if(!activeIdentity||!roleDeclared)throw new Error("This access authority could not be verified for the current authenticated IRPA identity. Please sign in again.");verifiedAuthority={...authority,authorityId,id:authorityId,role,active:true,verificationMode:"authenticated-identity-fallback"};}
try{window.sessionStorage.setItem("irpaActiveAccessAuthority",role);window.sessionStorage.setItem("irpaActiveAccessAuthorityId",authorityId);window.sessionStorage.setItem("irpaActiveAccessAuthorityRecord",JSON.stringify(verifiedAuthority));window.sessionStorage.removeItem("irpaPendingAccessAuthority");window.sessionStorage.setItem("irpaAccessAuthoritySelectionState",verifiedAuthority.verificationMode==="authenticated-identity-fallback"?"IDENTITY_VERIFIED_FALLBACK":"SERVER_VERIFIED");}catch{}setStoredAuthority(role);setActiveAuthorityLoginRequired(false);};useEffect(()=>{if(sessionAdministrator||!selectedAuthority)return;try{window.sessionStorage.setItem("irpaActiveAccessAuthority",selectedAuthority);window.sessionStorage.removeItem("irpaPendingAccessAuthority")}catch{}},[admin,selectedAuthority]);const[activeAuthorityLoginRequired,setActiveAuthorityLoginRequired]=useState(false);const effectiveAuthority=sessionAdministrator?"ADMINISTRATOR":selectedAuthority;const authorityProfile=selectedAuthority?{roles:[selectedAuthority],role:selectedAuthority}:null;const authorityEmployee=selectedAuthority&&employee&&(roleValues(employee?.roles||employee?.role).includes(selectedAuthority)||IT_ROLES.includes(selectedAuthority))?{...employee,roles:[selectedAuthority],role:selectedAuthority}:null;const financePortalAccess=sessionAdministrator||isFinancePortalMember(authorityProfile,authorityEmployee);const portalAccessFrozen=!sessionAdministrator&&!selectedAuthority;const[dataGateTarget,setDataGateTarget]=useState("");const[dataGateOpen,setDataGateOpen]=useState(false);const requestDataGate=target=>{if(!target){return}if(!IRPA_TEMPORARY_DATA_GATE_ENABLED){setActive(target);return}const currentPortal=PORTAL_ROLLER_GROUPS.find(([,children])=>active===children[0]||children.includes(active));const targetPortal=PORTAL_ROLLER_GROUPS.find(([,children])=>target===children[0]||children.includes(target));const samePortal=Boolean(currentPortal&&targetPortal&&currentPortal[0]===targetPortal[0]);if(samePortal){setActive(target);return}setDataGateTarget(target);setDataGateOpen(true)};const completeDataGate=mode=>{try{window.sessionStorage.setItem("irpaDataEnvironment",mode);window.sessionStorage.setItem("irpaDataEnvironmentTarget",dataGateTarget);window.sessionStorage.setItem("irpaDataEnvironmentSelectedAt",new Date().toISOString())}catch{}setDataGateOpen(false);setActive(dataGateTarget);};const[recoverySnapshot,setRecoverySnapshot]=useState(()=>readIrpaRecovery());const[recoveryMessage,setRecoveryMessage]=useState("");const[mobileNav,setMobileNav]=useState("");const[mobileTimeFormat,setMobileTimeFormat]=useState(()=>{try{return window.localStorage.getItem("irpa-mobile-time-format")==="12h"?"12h":"24h"}catch{return"24h"}});const[shellLiveNow,setShellLiveNow]=useState(()=>new Date());useEffect(()=>{const clockId=setInterval(()=>setShellLiveNow(new Date()),1000);return()=>clearInterval(clockId)},[]);const toggleMobileTimeFormat=()=>setMobileTimeFormat(previous=>{const next=previous==="24h"?"12h":"24h";try{window.localStorage.setItem("irpa-mobile-time-format",next)}catch{}return next});const liveDate=new Intl.DateTimeFormat("en-GB",{timeZone:"Africa/Nairobi",day:"2-digit",month:"long",year:"numeric"}).format(shellLiveNow);const liveDay=new Intl.DateTimeFormat("en-GB",{timeZone:"Africa/Nairobi",weekday:"long"}).format(shellLiveNow);const liveTime=new Intl.DateTimeFormat("en-GB",{timeZone:"Africa/Nairobi",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:mobileTimeFormat==="12h"}).format(shellLiveNow);const itPortalAccess=!sessionAdministrator&&IT_ROLES.includes(selectedAuthority);const procurementPortalAccess=sessionAdministrator||Boolean(selectedAuthority);const signatureProfileAccess=!sessionAdministrator&&(profile?.status==="Active"||employee?.status==="Active"||employee?.employmentStatus==="Active"||employee?.registrationStatus==="Activated");const meetingProfileAccess=!sessionAdministrator&&(profile?.status==="Active"||employee?.status==="Active"||employee?.employmentStatus==="Active"||employee?.registrationStatus==="Activated");const[active,setActive]=useState(()=>{const params=new URLSearchParams(window.location.search);const requested=params.get("adminModule");const induction=params.get("induction")==="1";return sessionAdministrator&&requested&&ALL_MODULES.includes(requested)?requested:induction?"Induction and Orientation":(IT_ROLES.includes(selectedAuthority)?"IT Operations":"Dashboard")}),modules=sessionAdministrator?ALL_MODULES:(portalAccessFrozen?["Dashboard"]:memberModules(profile,employee,selectedAuthority));useEffect(()=>{
  if(!user?.uid)return;
...
useEffect(()=>{
  ...
  const unsubscribe=observeAuthState(async u=>{
    ...
    try{
      ...
      // Administrators intentionally continue through the same authority
      // gateway as every other authenticated IRPA identity. The administrator
      // record is retained as an authoritative identity, but it no longer
      // bypasses authority selection.      const tokenResult=await u.getIdTokenResult(true).catch(()=>null);
      ...
      // A slow callable must not block the complete client authorization path.
      const loginContextPromise=Promise.race([
        resolveAuthenticatedLoginContext(),
        new Promise((_,reject)=>window.setTimeout(()=>reject(new Error("Server login context timed out.")),4500))
      ]).catch(error=>{
        console.warn("Server login context unavailable; using registered identity fallback.",error);
        return null;
      });
      ...
      // Keep the direct registered records in the same variables used by the
      // invitation/authorization normalization below. This was lost during the
      // gateway rebuild and caused non-Administrator sessions to throw a
      // ReferenceError before their member/employee profile could be committed.
      let m=memberDirect;
      let activeEmployee=employeeDirect;
      ...
      if(!memberAuthorized&&!employeeAuthorized&&!(Array.isArray(loginContext?.roles)&&loginContext.roles.length)){
        setError("Authentication succeeded, but no active IRPA authorization record was resolved.");
        setProfile(null);
        return;
      }
      ...
      const claimsOnlyAuthorized=serverContextRoles.length>0;
      if(!activeMember && !activeEmployee && !claimsOnlyAuthorized){
        setError("This account has no active IRPA enrollment record or server-authorized IRPA role.");
        setProfile(null);
        return;
      }
      // Employees are enrolled from the Employees Register and do not require
      // a duplicate members/{uid} authorization document. If the client register
      // lookup is unavailable but server-controlled IRPA claims are present, keep
      // the authenticated role context and allow the role gateway to continue.
