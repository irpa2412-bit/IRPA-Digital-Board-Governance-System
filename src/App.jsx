...
function LiveWeatherPanel(){
 ...
 const loadWeather=async()=>{try{setError("");const url="https://api.open-meteo.com/v1/forecast?latitude=-2.73319&longitude=36.69773&current=temperature_2m,relative_humidity_2m,apparent_temperature,weather_code,wind_speed_10m&hourly=temperature_2m,precipitation_probability,weather_code&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max&timezone=Africa%2FNairobi&forecast_days=5";const response=await fetch(url,{cache:"no-store"});if(!response.ok)throw new Error("Weather service unavailable");const data=await response.json();if(!data?.current||!data?.daily)throw new Error("Incomplete weather response");setWeather(data)}catch(x){setError(x?.message||"Unable to load live weather")}};
 useEffect(()=>{loadWeather();const id=window.setInterval(loadWeather,15*60*1000);return()=>window.clearInterval(id)},[]);
 const condition=code=>({0:"Clear sky",1:"Mainly clear",2:"Partly cloudy",3:"Overcast",45:"Fog",48:"Depositing rime fog",51:"Light drizzle",53:"Moderate drizzle",55:"Dense drizzle",61:"Slight rain",63:"Moderate rain",65:"Heavy rain",71:"Slight snow",73:"Moderate snow",75:"Heavy snow",80:"Rain showers",81:"Moderate rain showers",82:"Violent rain showers",95:"Thunderstorm",96:"Thunderstorm with hail",99:"Thunderstorm with heavy hail"}[code]||"Weather conditions");
 const icon=code=>code===0?"☀️":code<=3?"⛅":code<=48?"🌫️":code<=55?"🌦️":code<=65?"🌧️":code<=75?"❄️":code<=82?"🌦️":"⛈️";
 const formatDay=value=>new Intl.DateTimeFormat("en-GB",{weekday:"short",day:"2-digit",month:"short",timeZone:"Africa/Nairobi"}).format(new Date(value+"T12:00:00"));
 const current=weather?.current;
 return <div className="live-weather-content" aria-live="polite">
   <div className="live-weather-current">
    <div className="live-weather-current-main"><span className="live-weather-icon" aria-hidden="true">{current?icon(current.weather_code):"☁️"}</span><div><strong>{current?Math.round(current.temperature_2m)+"°C":"Live weather"}</strong><span>{current?condition(current.weather_code):error||"Loading live forecast..."}</span><small>Longido, Tanzania · feels like {current?Math.round(current.apparent_temperature)+"°C":"—"}</small></div></div>
    <div className="live-weather-metrics"><span>💧 {current?current.relative_humidity_2m:"—"}%</span><span>💨 {current?Math.round(current.wind_speed_10m):"—"} km/h</span><span>🌧️ {weather?.daily?.precipitation_probability_max?.[0]??"—"}% today</span></div>
   </div>
   <div className="live-weather-days">{weather?.daily?.time?.slice(0,4).map((day,index)=><div className="live-weather-day" key={day}><strong>{index===0?"Today":formatDay(day)}</strong><span>{icon(weather.daily.weather_code[index])} {condition(weather.daily.weather_code[index])}</span><b>{Math.round(weather.daily.temperature_2m_max[index])}° / {Math.round(weather.daily.temperature_2m_min[index])}°</b><small>Rain {weather.daily.precipitation_probability_max[index]}%</small></div>)||<div className="live-weather-loading">Loading forecast…</div>}</div>
   <div className="live-weather-attribution"><span>LIVE FORECAST · updates automatically</span><a href="https://open-meteo.com/" target="_blank" rel="noopener noreferrer">Weather data by Open-Meteo.com</a></div>
 </div>;
}
