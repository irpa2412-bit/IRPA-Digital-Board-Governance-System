...
export default function ExternalAuditorGateway(){const[email,setEmail]=useState("");const[password,setPassword]=useState("");const[mode,setMode]=useState("login");const[busy,setBusy]=useState(false);const[message,setMessage]=useState("");const[auditor,setAuditor]=useState(null);async function verify(user){const clean=String(user?.email||"").trim().toLowerCase();if(!clean)throw new Error("A valid auditor email is required.");const snap=await getDoc(doc(db,"auditorProfiles",clean));if(!snap.exists()||snap.data()?.active!==true)throw new Error("This email is not authorized as an active IRPA External Auditor. Contact the IRPA Administrator.");setAuditor({...snap.data(),uid:user.uid,email:clean});}async function submit(e){e.preventDefault();setBusy(true);setMessage("");try{if(mode==="register"){await registerWithEmail(email.trim().toLowerCase(),password);setMessage("Account created. Verify your email, then return here to sign in. External Auditor access still requires an active IRPA auditor authorization.");}else{const u=await loginWithEmail(email.trim().toLowerCase(),password);await verify(u);}}catch(x){setMessage(x?.message||"External Auditor authentication failed.");}finally{setBusy(false)}}if(auditor)return <><div className="app-shell"><main className="content-area" style={{width:"100%"}}><ExternalAuditorPortal/></main></div></>;return <main className="auth-screen"><section className="auth-card"><div className="auth-brand"><div className="brand-kicker">IRPA</div><h1>External Auditor Portal</h1><p>Read-only independent access to IRPA governance workflow evidence and reporting.</p></div><div className="auth-divider"><span>External Auditor Access</span></div><form onSubmit={submit}><label>Email address</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} required autoComplete="email"/><label>Password</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} required minLength={8} autoComplete="current-password"/><button type="submit" disabled={busy}>{busy?"Authenticating…":"Enter External Auditor Portal"}</button></form>{message&&<div className="auth-message">{message}</div>}<div className="auth-links"><button className="text-button" type="button" onClick={async()=>{if(!email.trim()){setMessage("Enter your auditor email address first.");return}try{await sendPasswordReset(email.trim());setMessage("Password reset email sent.");}catch(x){setMessage(x?.message||"Unable to send password reset email.")}}}>Forgot password?</button><button className="text-button" type="button" onClick={()=>setMode(mode==="login"?"register":"login")}>{mode==="login"?"Create account":"Return to sign in"}</button></div></section></main>}
