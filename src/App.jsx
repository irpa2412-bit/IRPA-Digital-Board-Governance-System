...
function Dashboard({user,admin,employee,profile,onNavigate}){
 ...
 return <div className="page dashboard-command">
   <div className="dashboard-desktop-view">
    ...
    <div className="dashboard-live-date-slot"><div className="desktop-date-card open-portal-panel" aria-label="Live date and time"><div className="live-date-time-content"><span>LIVE DATE & TIME</span><strong>{liveDay}</strong><small>{liveDate} · {liveTime}</small><em className="desktop-date-workspace">IRPA Digital Governance Workspace</em></div><div className="live-date-stat-buttons" aria-label="Dashboard member and meeting shortcuts">{cards.slice(0,2).map(([label,value,sub,target])=><button className="desktop-blue-nav-button" key={label} type="button" onClick={()=>go(target)}><span className="quote4-stat-icon">{iconFor(label)}</span><div><strong>{busy?"—":value}</strong><b>{label}</b><small>{sub}</small></div><i>›</i></button>)}</div></div></div><section className="dashboard-quote4-stats">{cards.slice(2).map(([label,value,sub,target])=><button className="desktop-blue-nav-button" key={label} type="button" onClick={()=>go(target)}><span className="quote4-stat-icon">{iconFor(label)}</span><div><strong>{busy?"—":value}</strong><b>{label}</b><small>{sub}</small></div><i>›</i></button>)}</section>
