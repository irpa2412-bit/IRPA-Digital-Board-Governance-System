...
function AuthorityCapacityGate({categories,onConfirm,onSignOut}){const normalizedCategories=[...new Set((Array.isArray(categories)?categories:[]).map(value=>String(value||"").trim()).filter(Boolean))];const dualRole=normalizedCategories.length>1;const[selected,setSelected]=useState("");const[busy,setBusy]=useState(false);const[feedback,setFeedback]=useState("");const choose=value=>{const next=String(value||"").trim();if(!normalizedCategories.includes(next)||busy)return;setSelected(next);setFeedback("");};const verify=async()=>{if(!selected||busy)return;if(!normalizedCategories.includes(selected)){setFeedback("The declared role is no longer present in the verified account roles. Select an available role again.");return}if(!auth.currentUser?.uid){setFeedback("The authenticated IRPA session is no longer available. Sign in again.");return}setBusy(true);setFeedback("");try{await Promise.resolve(onConfirm(selected));}catch(error){console.error("IRPA role declaration verification failed:",error);setFeedback(error?.message||"Role declaration could not be completed. Please select the role again.");setBusy(false);}};return <div className="authority-capacity-gate" role="dialog" aria-modal="true" aria-labelledby="authority-capacity-gate-title" aria-describedby="authority-capacity-gate-description"><section className="authority-capacity-card">{normalizedCategories.length===0?<><div className="authority-capacity-kicker">IRPA-DBGS · ACCESS VERIFICATION</div><h1 id="authority-capacity-gate-title">Role Verification Unavailable</h1><p id="authority-capacity-gate-description">No verified operating role was returned for this authenticated account. Navigation remains locked until an authorised role is available.</p><div className="authority-capacity-feedback error" role="alert"><strong>VERIFICATION BLOCKED</strong><span>Do not continue without a registered role. Sign out and use an account with an active IRPA authorization profile.</span></div><button type="button" className="authority-capacity-signout" onClick={onSignOut}>Sign Out</button></>:<><div className="authority-capacity-kicker">IRPA-DBGS · ACCESS VERIFICATION</div><h1 id="authority-capacity-gate-title">{dualRole?"Declare Your Login Role":"Confirm Your Login Role"}</h1><p id="authority-capacity-gate-description">{dualRole?"This account has more than one registered IRPA role. You must explicitly declare which role you are logging in as for this session. Only the declared role will control the session's visible operating functions.":"Select the registered IRPA role you will use for this session. Server-side authorization remains authoritative."}</p>{dualRole&&<div className="authority-capacity-feedback" role="note"><strong>DUAL-ROLE ACCOUNT · EXPLICIT DECLARATION REQUIRED</strong><span>Your other registered role remains on your account but is not selected for this login session.</span></div>}<div className="authority-capacity-status"><span className="authority-capacity-status-dot" aria-hidden="true"></span><div><strong>{selected?"ROLE DECLARED FOR THIS LOGIN":"LOGIN ROLE NOT YET DECLARED"}</strong><small>{selected?"Logging in as: "+selected:"Select exactly one registered role below."}</small></div></div><div className="authority-capacity-options" role="radiogroup" aria-label={dualRole?"Declare login role":"Select login role"}>{normalizedCategories.map(category=><button key={category} type="button" className={selected===category?"authority-capacity-option selected":"authority-capacity-option"} onClick={()=>choose(category)} aria-pressed={selected===category} role="radio" aria-checked={selected===category} disabled={busy}><span>{category}</span><small>{selected===category?"Declared for this login":"Available registered role"}</small></button>)}</div>{feedback&&<div className="authority-capacity-feedback error" role="alert" aria-live="assertive"><strong>ROLE DECLARATION MESSAGE</strong><span>{feedback}</span></div>}<button type="button" className="authority-capacity-confirm" disabled={!selected||busy} onClick={verify}>{busy?"Verifying Declared Role…":dualRole?"Declare Role &amp; Enter Dashboard":"Confirm Role &amp; Enter Dashboard"}</button><button type="button" className="authority-capacity-signout" onClick={onSignOut} disabled={busy}>Sign Out</button><small className="authority-capacity-note">This declaration selects the operating role for this authenticated session; it does not grant new privileges. Existing server-side role and access controls remain authoritative.</small></>}</section></div>}function Shell({user,profile,admin,employee,inductionComplete,onInductionComplete}){const authorityCategories=resolveLoginCategories(profile,employee,admin);const[storedAuthority,setStoredAuthority]=useState(()=>{try{return window.sessionStorage.getItem("irpaActiveAccessAuthority")||""}catch{return""}});const selectedAuthority=authorityCategories.length===1&&admin&&authorityCategories[0]==="Administrator"?"Administrator":authorityCategories.includes(storedAuthority)?storedAuthority:"";const sessionAdministrator=selectedAuthority==="Administrator";const setSelectedAuthority=authority=>{if(sessionAdministrator||!authorityCategories.includes(authority)||authority===selectedAuthority)return;try{window.sessionStorage.setItem("irpaPendingAccessAuthority",authority);window.sessionStorage.removeItem("irpaActiveAccessAuthority")}catch{}setStoredAuthority("");setActiveAuthorityLoginRequired(true);setMobileNav("");setActive("Dashboard");void logout();};const confirmInitialAuthority=async authority=>{if(sessionAdministrator||!authorityCategories.includes(authority))return;const normalizedAuthority=String(authority||"").trim();let verificationState="SERVER_VERIFIED";try{const context=await Promise.race([resolveAuthenticatedLoginContext(),new Promise((_,reject)=>window.setTimeout(()=>reject(new Error("Server role verification timed out.")),5000))]);const serverRoles=[...(Array.isArray(context?.roles)?context.roles:[])];const serverAllows=normalizedAuthority==="Administrator"&&context?.administrator?.active===true||serverRoles.includes(normalizedAuthority);if(!serverAllows)throw new Error("The selected IRPA capacity is not present in the server-authorized login context.");}catch(error){console.warn("Server role resolver unavailable; using the already verified registered-role set without granting additional privileges.",error);verificationState="LOCAL_REGISTER_VERIFIED_PENDING_SERVER";}try{window.sessionStorage.setItem("irpaActiveAccessAuthority",normalizedAuthority);window.sessionStorage.removeItem("irpaPendingAccessAuthority");window.sessionStorage.setItem("irpaAccessAuthoritySelectionState",verificationState);}catch{}setStoredAuthority(normalizedAuthority);setActiveAuthorityLoginRequired(false);};useEffect(()=>{if(sessionAdministrator||!selectedAuthority)return;try{window.sessionStorage.setItem("irpaActiveAccessAuthority",selectedAuthority);window.sessionStorage.removeItem("irpaPendingAccessAuthority")}catch{}},[admin,selectedAuthority]);const[activeAuthorityLoginRequired,setActiveAuthorityLoginRequired]=useState(false);const effectiveAuthority=sessionAdministrator?"ADMINISTRATOR":selectedAuthority;const authorityProfile=selectedAuthority?{roles:[selectedAuthority],role:selectedAuthority}:null;const authorityEmployee=selectedAuthority&&employee&&(roleValues(employee?.roles||employee?.role).includes(selectedAuthority)||IT_ROLES.includes(selectedAuthority))?{...employee,roles:[selectedAuthority],role:selectedAuthority}:null;const financePortalAccess=sessionAdministrator||isFinancePortalMember(authorityProfile,authorityEmployee);const portalAccessFrozen=!sessionAdministrator&&!selectedAuthority;const[dataGateTarget,setDataGateTarget]=useState("");const[dataGateOpen,setDataGateOpen]=useState(false);const requestDataGate=target=>{if(!target){return}if(!IRPA_TEMPORARY_DATA_GATE_ENABLED){setActive(target);return}const currentPortal=PORTAL_ROLLER_GROUPS.find(([,children])=>active===children[0]||children.includes(active));const targetPortal=PORTAL_ROLLER_GROUPS.find(([,children])=>target===children[0]||children.includes(target));const samePortal=Boolean(currentPortal&&targetPortal&&currentPortal[0]===targetPortal[0]);if(samePortal){setActive(target);return}setDataGateTarget(target);setDataGateOpen(true)};const completeDataGate=mode=>{try{window.sessionStorage.setItem("irpaDataEnvironment",mode);window.sessionStorage.setItem("irpaDataEnvironmentTarget",dataGateTarget);window.sessionStorage.setItem("irpaDataEnvironmentSelectedAt",new Date().toISOString())}catch{}setDataGateOpen(false);setActive(dataGateTarget);};const[recoverySnapshot,setRecoverySnapshot]=useState(()=>readIrpaRecovery());const[recoveryMessage,setRecoveryMessage]=useState("");const[mobileNav,setMobileNav]=useState("");const[mobileTimeFormat,setMobileTimeFormat]=useState(()=>{try{return window.localStorage.getItem("irpa-mobile-time-format")==="12h"?"12h":"24h"}catch{return"24h"}});const[shellLiveNow,setShellLiveNow]=useState(()=>new Date());useEffect(()=>{const clockId=setInterval(()=>setShellLiveNow(new Date()),1000);return()=>clearInterval(clockId)},[]);const toggleMobileTimeFormat=()=>setMobileTimeFormat(previous=>{const next=previous==="24h"?"12h":"24h";try{window.localStorage.setItem("irpa-mobile-time-format",next)}catch{}return next});const liveDate=new Intl.DateTimeFormat("en-GB",{timeZone:"Africa/Nairobi",day:"2-digit",month:"long",year:"numeric"}).format(shellLiveNow);const liveDay=new Intl.DateTimeFormat("en-GB",{timeZone:"Africa/Nairobi",weekday:"long"}).format(shellLiveNow);const liveTime=new Intl.DateTimeFormat("en-GB",{timeZone:"Africa/Nairobi",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:mobileTimeFormat==="12h"}).format(shellLiveNow);const itPortalAccess=!sessionAdministrator&&IT_ROLES.includes(selectedAuthority);const procurementPortalAccess=sessionAdministrator||isProcurementPortalMember(authorityProfile,authorityEmployee);const signatureProfileAccess=!sessionAdministrator&&(profile?.status==="Active"||employee?.status==="Active"||employee?.employmentStatus==="Active"||employee?.registrationStatus==="Activated");const meetingProfileAccess=!sessionAdministrator&&(profile?.status==="Active"||employee?.status==="Active"||employee?.employmentStatus==="Active"||employee?.registrationStatus==="Activated");const[active,setActive]=useState(()=>{const params=new URLSearchParams(window.location.search);const requested=params.get("adminModule");const induction=params.get("induction")==="1";return sessionAdministrator&&requested&&ALL_MODULES.includes(requested)?requested:induction?"Induction and Orientation":(IT_ROLES.includes(selectedAuthority)?"IT Operations":"Dashboard")}),modules=sessionAdministrator?ALL_MODULES:(portalAccessFrozen?["Dashboard"]:memberModules(profile,employee,selectedAuthority));useEffect(()=>{
  if(!user?.uid)return;
