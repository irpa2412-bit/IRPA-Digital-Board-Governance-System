import React,{useEffect,useMemo,useState}from"react";import Invitations from"./pages/Invitations";import Employees from"./pages/Employees";import BoardMembers from"./pages/BoardMembers";import EmployeePayments from"./pages/EmployeePayments";import Meetings from"./pages/Meetings";import Resolutions from"./pages/Resolutions";import Voting from"./pages/Voting";import OperationalGateways from"./pages/OperationalGateways";import OperationalGatewaysParticipants from"./pages/OperationalGatewaysParticipants";import AuthorizationApprovals from"./pages/AuthorizationApprovals";import SignaturePlatformWithUpload from"./pages/SignaturePlatformWithUpload";import FinancePortfolio from"./pages/FinancePortfolio";import Settings from"./pages/Settings";import ModuleInterlinkBar from"./components/ModuleInterlinkBar";import{observeAuthState,loginWithEmail,loginWithGoogle,registerWithEmail,sendPasswordReset,sendAdminMagicLink,isMagicLink,completeMagicLink,logout}from"./firebase/auth";import{getAdminProfile,getCurrentMemberProfile,getCurrentEmployeeProfile,getRecords,COLLECTIONS}from"./firebase/data";import{getSignatureEnvelope}from"./firebase/signaturePlatform";import{provisionCurrentMemberFromInvitationV2}from"./firebase/invitationWorkflow";import{auth}from"./firebase/config";
const NAV={GOVERNANCE:["Dashboard","Board Members Registration","Members & Personnel","Invitations","Participants","Meetings","Meeting Room","Resolutions","Voting","Actions","Documents","Signature Platform","Decisions","Risk Register","Authorization & Approvals","Employee Payments"],FINANCE:["Finance Portfolio","Procurement"],EVIDENCE:["Reports","Audit Trail"],SYSTEM:["Settings"]};const ALL_MODULES=Object.values(NAV).flat();const BASE_MEMBER_MODULES=["Dashboard","Meetings","Meeting Room","Resolutions","Voting","Actions","Documents","Signature Platform","Decisions","Authorization & Approvals","Employee Payments","Reports"];const FINANCE_ROLES=["Finance Personnel","Finance Manager","Accountant","Finance Officer","Executive Director"];const HR_ROLES=["Executive Director","Director Human Resources","HR Manager"];const CONFIGURED_MODULES=["Meeting Room","Actions","Documents","Decisions","Risk Register","Procurement","Reports","Audit Trail"];
...
function MemberActivationScreen({invitationId}) {
  ...
  async function activate(e){
    ...
    setBusy(true);
    try{
      const {registerWithEmail,loginWithEmail,logout}=await import("./firebase/auth");
      try{
        await registerWithEmail(clean,password,{verify:false});
      }catch(x){
        if(x?.code==="auth/email-already-in-use") await loginWithEmail(clean,password);
        else throw x;
      }
      await provisionCurrentMemberFromInvitationV2(invitationId);
      await logout();
      window.location.replace(window.location.origin+"/?activated=1");
    }catch(x){setMessage(x.message||"Unable to activate the IRPA account.");}
...
useEffect(()=>observeAuthState(async u=>{setUser(u);setProfile(undefined);setEmployee(null);setError("");if(!u){setProfile(null);return}try{
...
}magic()},[]);const memberInviteId=new URLSearchParams(window.location.search).get("memberInvite");
if(user===undefined)return <Loading/>;
if(!user&&memberInviteId)return <MemberActivationScreen invitationId={memberInviteId}/>;
...
return <Shell user={user} profile={profile} admin={profile.authorizationType==="administrator"} employee={employee}/>;
}
