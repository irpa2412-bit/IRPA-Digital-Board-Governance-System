...
useEffect(()=>observeAuthState(async u=>{setUser(u);setProfile(undefined);setEmployee(null);setInductionComplete(false);setError("");if(!u){setProfile(null);return}try{
  // The primary administrator is bootstrapped only after successful Google authentication.
  // This is isolated to authentication/authorization and does not alter governance modules.
  if(String(u.email||"").trim().toLowerCase()==="irpa2412@gmail.com"){
    try{await bootstrapPrimaryAdministrator();}catch(bootstrapError){console.warn("Primary administrator bootstrap did not complete; checking existing administrator profile.",bootstrapError);}
  }
