...
function AuthorityCapacityGate({categories,onConfirm,onSignOut}){const normalizedCategories=[...new Set((Array.isArray(categories)?categories:[]).map(value=>String(value||"").trim()).filter(Boolean))];const[selected,setSelected]=useState("");const[busy,setBusy]=useState(false);const[feedback,setFeedback]=useState("");const choose=value=>{const next=String(value||"").trim();if(!normalizedCategories.includes(next)||busy)return;setSelected(next);setFeedback("");};const verify=async()=>{if(!selected||busy)return;if(!normalizedCategories.includes(selected)){setFeedback("The selected operating authority is no longer present in the verified account categories. Select an available authority again.");return}if(!auth.currentUser?.uid){setFeedback("The authenticated IRPA session is no longer available. Sign in again.");return}setBusy(true);setFeedback("");try{await Promise.resolve(onConfirm(selected));}catch(error){console.error("IRPA authority verification failed:",error);setFeedback(error?.message||"Authority verification could not be completed. Please select the authority again.");setBusy(false);}};return <div className="authority-capacity-gate" role="dialog" aria-modal="true" aria-labelledby="authority-capacity-gate-title" aria-describedby="authority-capacity-gate-description"><section className="authority-capacity-card">{normalizedCategories.length===0?<><div className="authority-capacity-kicker">IRPA-DBGS · ACCESS VERIFICATION</div><h1 id="authority-capacity-gate-title">Authority Verification Unavailable</h1><p id="authority-capacity-gate-description">No verified operating authority was returned for this authenticated account. Navigation remains locked until an authorised capacity is available.</p><div className="authority-capacity-feedback error" role="alert"><strong>VERIFICATION BLOCKED</strong><span>Do not continue without a registered authority. Sign out and use an account with an active IRPA authorization profile.</span></div><button type="button" className="authority-capacity-signout" onClick={onSignOut}>Sign Out</button></>:<><div className="authority-capacity-kicker">IRPA-DBGS · ACCESS VERIFICATION</div><h1 id="authority-capacity-gate-title">Verify Your Operating Authority</h1><p id="authority-capacity-gate-description">Select one operating capacity already registered to this account. The selected capacity controls this session's visible operating functions; server-side authorization remains authoritative.</p><div className="authority-capacity-status"><span className="authority-capacity-status-dot" aria-hidden="true"></span><div><strong>{selected?"AUTHORITY READY FOR VERIFICATION":"AUTHORITY SELECTION REQUIRED"}</strong><small>{selected?"Selected: "+selected:"Choose a registered capacity below."}</small></div></div><div className="authority-capacity-options" role="listbox" aria-label="Registered operating authorities">{normalizedCategories.map(category=><button key={category} type="button" className={selected===category?"authority-capacity-option selected":"authority-capacity-option"} onClick={()=>choose(category)} aria-pressed={selected===category} disabled={busy}><span>{category}</span><small>{selected===category?"Selected · ready to verify":"Registered authority"}</small></button>)}</div>{feedback&&<div className="authority-capacity-feedback error" role="alert" aria-live="assertive"><strong>VERIFICATION MESSAGE</strong><span>{feedback}</span></div>}<button type="button" className="authority-capacity-confirm" disabled={!selected||busy} onClick={verify}>{busy?"Verifying Authority…":"Verify Authority &amp; Enter Dashboard"}</button><button type="button" className="authority-capacity-signout" onClick={onSignOut} disabled={busy}>Sign Out</button><small className="authority-capacity-note">The verifier does not grant new privileges. It confirms the session is using one of the account's registered authorities. Existing server-side role and access controls remain authoritative.</small></>}</section></div>}
