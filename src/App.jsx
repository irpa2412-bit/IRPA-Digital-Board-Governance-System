...
function Dashboard({user,admin,employee,profile,onNavigate}){
 ...
 useEffect(()=>{let live=true;(async()=>{
   try{
    if(admin){
      const names=[COLLECTIONS.employees,COLLECTIONS.members,COLLECTIONS.meetings,COLLECTIONS.authorizationRequests,COLLECTIONS.actions,COLLECTIONS.risks,COLLECTIONS.reports,COLLECTIONS.documents,COLLECTIONS.signatures,COLLECTIONS.audit];
      const v=await Promise.all(names.map(n=>getRecords(n).catch(()=>[])));
      if(live){
       setStats({employees:v[0].length,members:v[1].length,meetings:v[2].length,authorizations:v[3].filter(x=>["Submitted","Under Review"].includes(x.status)).length,actions:v[4].filter(x=>!["Completed","Cancelled"].includes(x.status)).length,risks:v[5].filter(x=>!["Closed","Mitigated"].includes(x.status)).length,payments:0,signatures:v[8].filter(x=>!["Completed","Signed","Declined"].includes(x.status)).length,reports:v[6].filter(x=>!["Archived"].includes(x.status)).length,documents:v[7].length,audit:v[9].length});
       const rows=[...v[9].map(x=>({...x,_kind:"Audit"})),...v[3].map(x=>({...x,_kind:"Authorization"})),...v[2].map(x=>({...x,_kind:"Meeting"})),...v[4].map(x=>({...x,_kind:"Action"})),...v[7].map(x=>({...x,_kind:"Document"})),...v[8].map(x=>({...x,_kind:"Signature"}))];
 ...
 const quick=admin?[["Create Authorization","Authorization & Approvals","Create and track governance authorizations","check"],["New Payment","Employee Payments","Start an employee payment workflow","finance"],["Upload Document","Documents","Upload and manage authorised documents","document"],["View Reports","Reports","Open governance reports and analytics","analytics"]]:[["Meetings","Meetings","Schedule and manage assigned meetings","calendar"],["Documents","Documents","Access authorised governance documents","document"],["Finance","Finance Portfolio","Access authorised finance workflows","finance"],["Authorizations","Authorization & Approvals","Review assigned approvals","check"],["Signatures","Signature Platform","Complete assigned signatures","signature"],["Reports","Reports","View authorised reports","analytics"]];
 ...
 const activityLabel=x=>x._kind==="Audit"?(x.action||"Governance audit event"):x._kind==="Authorization"?(x.title||x.description||x.referenceNumber||"Authorization request"):x._kind==="Meeting"?(x.title||x.subject||"Meeting"):x._kind==="Payment"?(x.referenceNumber||x.type||"Payment request"):x._kind==="Document"?(x.title||x.name||"Governance document"):x._kind==="Signature"?(x.title||x.documentName||"Signature workflow"):(x.title||x.description||"Action");
 const activityStatus=x=>x.status||x.workflowStage||(x._kind==="Audit"?"Recorded":"Recorded");
 const activityDate=x=>{const v=x.date||x.updatedAt||x.createdAt;if(v?.toDate)return v.toDate().toLocaleString(undefined,{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"});if(v instanceof Date)return v.toLocaleString();return String(v||"—")};
 const activityTarget=x=>{if(x._kind==="Audit"){const c=String(x.collection||"");if(c==="members"||c==="employees")return "Members & Personnel";if(c==="meetings")return "Meetings";if(c==="authorizationRequests")return "Authorization & Approvals";if(c==="documents")return "Documents";if(c==="signatures")return "Signature Platform";if(c==="actions")return "Actions";if(c==="reports")return "Reports";if(c==="staffPaymentRequests")return "Employee Payments";if(c.startsWith("finance"))return "Finance Portfolio";if(c.startsWith("procurement"))return "Procurement";if(c==="audit")return "Audit Trail";return "Reports"}if(x._kind==="Authorization")return "Authorization & Approvals";if(x._kind==="Meeting")return "Meetings";if(x._kind==="Payment")return "Employee Payments";if(x._kind==="Document")return "Documents";if(x._kind==="Signature")return "Signature Platform";return "Actions"};
 ...
 return <div className="page dashboard-command">
   <div className="dashboard-desktop-view">
    ...
    <section className="dashboard-quote4-main">
      <div className="dashboard-quote4-activity panel">
       ...
       <div className="quote4-activity-table"><div className="quote4-activity-header" aria-hidden="true"><span>Date</span><span>Activity</span><span>Reference</span><span>Status</span><span></span></div>{activity.length?activity.map((x,i)=>{const status=activityStatus(x);const target=activityTarget(x);return <button type="button" key={x.id||i} aria-label={`Open ${activityLabel(x)}`} onClick={()=>go(target)}><span>{activityDate(x)}</span><strong>{activityLabel(x)}</strong><small>{x.referenceNumber||x.reference||x.recordId||x.employeeNumber||x.collection||x._kind}</small><em className={activityStatusClass(status)}>{status}</em><b>›</b></button>}):<div className="quote4-empty"><strong>Governance workspace is ready.</strong><span>No activity records are currently visible in this dashboard view.</span><div className="quote4-empty-actions"><button type="button" onClick={()=>go("Members & Personnel")}>Members</button><button type="button" onClick={()=>go("Meetings")}>Meetings</button><button type="button" onClick={()=>go("Authorization & Approvals")}>Authorizations</button><button type="button" onClick={()=>go("Documents")}>Documents</button></div></div>}</div>
      </div>
   ...
   <div className="dashboard-mobile-view" aria-label="Mobile IRPA governance dashboard">
    ...
    <section className="mobile-quote1-stats">{cards.map(([label,value,sub,target])=><button className="mobile-blue-nav-button" key={label} type="button" onClick={()=>go(target)}><span>{iconFor(label)}</span><strong>{busy?"—":value}</strong><b>{label}</b><small>{sub}</small><i>›</i></button>)}</section>
    <section className="mobile-quote1-quick"><div className="mobile-quote1-heading"><h2>Quick Access</h2><button className="mobile-blue-nav-button" type="button" onClick={()=>go("Meetings")}>View all ›</button></div><div className="mobile-quote1-grid">{quick.slice(0,6).map(([label,target,desc,ico])=><button className="mobile-blue-nav-button" key={target} type="button" onClick={()=>go(target)}><span>{iconFor(ico)}</span><strong>{label}</strong></button>)}</div></section>
   </div>
 </div>
