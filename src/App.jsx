...
function resolveLoginCategories(profile,employee,admin){if(admin)return["ADMINISTRATOR"];const values=[...(Array.isArray(profile?.roles)?profile.roles:[]),...(Array.isArray(profile?.assignedRoles)?profile.assignedRoles:[]),...(Array.isArray(profile?.selectedRoles)?profile.selectedRoles:[]),profile?.role,employee?.role,...(Array.isArray(employee?.roles)?employee.roles:[])].map(v=>String(v||"").trim()).filter(Boolean);return [...new Set(values)];}
