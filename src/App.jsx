function MobilePortalIndicator({active,modules,onNavigate}){const options=[...new Set(["Dashboard",...modules])].filter(Boolean);return <div className="mobile-portal-indicator" aria-label="Current portal navigator"><div className="mobile-portal-indicator-label"><span>PORTAL</span><strong>{displayModuleName(active)}</strong></div><select aria-label="Select portal" value={active} onChange={e=>onNavigate(e.target.value)}>{options.map(item=><option key={item} value={item}>{displayModuleName(item)}</option>)}</select><span className="mobile-portal-indicator-chevron" aria-hidden="true">⌄</span></div>}
