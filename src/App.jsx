...
function LoginCategoryBadges({profile,employee,admin}){const categories=resolveLoginCategories(profile,employee,admin);const memberNumber=String(profile?.memberNumber||"").trim();const employeeNumber=String(employee?.employeeNumber||"").trim();return <div aria-label="Login category" style={{marginTop:4}}><span style={{fontSize:10,fontWeight:800,letterSpacing:".08em",opacity:.72,display:"block"}}>AUTHORIZED USER · ACCESS CATEGORIES</span><div style={{display:"flex",flexWrap:"wrap",gap:7,marginTop:4,alignItems:"flex-start"}}>{categories.length?categories.map(category=>{const identifier=category==="Board Secretary"?memberNumber:category==="Executive Director"?employeeNumber:"";return <div key={category} style={{display:"inline-flex",flexDirection:"column",alignItems:"flex-start",gap:3}}><span style={{display:"inline-flex",alignItems:"center",padding:"3px 7px",borderRadius:999,fontSize:11,fontWeight:700,border:"1px solid currentColor",lineHeight:1.2}}>{category}</span>{identifier&&<small style={{fontSize:10,lineHeight:1.2,opacity:.82,paddingLeft:7}}>{identifier}</small>}</div>}):<span style={{fontSize:11,opacity:.72}}>Authorized IRPA User</span>}</div><small style={{display:"block",fontSize:9,lineHeight:1.25,opacity:.62,marginTop:6}}>Access categories only — the current signing authority is selected separately in the Signing Identity · Registered Capacity panel.</small></div>}
