...
function AuthScreen(){const params=new URLSearchParams(window.location.search);const inductionMode=params.get("induction")==="1";const[mode,setMode]=useState("login"),[email,setEmail]=useState(()=>params.get("adminEmail")||""),[password,setPassword]=useState(""),[message,setMessage]=useState(""),[busy,setBusy]=useState(false),[authCooldown,setAuthCooldown]=useState(0),[passwordPopup,setPasswordPopup]=useState(null),[resetOpen,setResetOpen]=useState(false),[resetStep,setResetStep]=useState("REQUEST"),[resetConfirmation,setResetConfirmation]=useState(null),[resetPhone,setResetPhone]=useState(""),[resetCode,setResetCode]=useState("");const notifyPasswordPopup=(text,type="info")=>{setPasswordPopup({text,type});window.setTimeout(()=>setPasswordPopup(null),3600)};useEffect(()=>{if(authCooldown<=0)return;const t=window.setInterval(()=>setAuthCooldown(v=>v>0?v-1:0),1000);return()=>window.clearInterval(t)},[authCooldown]);async function verifyRole(user){
  ...
  const withTimeout=(promise,ms,label)=>Promise.race([promise,new Promise((_,reject)=>window.setTimeout(()=>reject(new Error(label)),ms))]);
  const administratorPromise=getAdminProfile(user.uid).catch(()=>null);
  const contextPromise=Promise.race([
    resolveAuthenticatedLoginContext(),
    new Promise((_,reject)=>window.setTimeout(()=>reject(new Error("Server authorization check timed out.")),4500))
  ]).catch(error=>{
    console.warn("Server login context unavailable; continuing with registered Firebase identity fallback.",error);
    return null;
  });
  const registeredIdentityName=String(user?.displayName||"").trim();
  const registerPromise=Promise.all([
    ...
    withTimeout(getCurrentEmployeeProfile({fallbackName:registeredIdentityName}).catch(()=>null),5000,"Employee authorization lookup timed out.")
  ]).catch(error=>{
  ...
  const employeeAuthorized=Boolean(resolvedEmployee)&&(["active","activated"].includes(String(resolvedEmployee?.status||"").trim().toLowerCase())||String(resolvedEmployee?.employmentStatus||"").trim().toLowerCase()==="active"||String(resolvedEmployee?.registrationStatus||"").trim().toLowerCase()==="activated");
  const tokenAuthorized=Array.isArray((await user.getIdTokenResult(true).catch(()=>null))?.claims?.irpaRoles)&&((await user.getIdTokenResult(true).catch(()=>null))?.claims?.irpaRoles||[]).length>0;
  const serverAuthorized=Array.isArray(context?.roles)&&context.roles.length>0;
  if(administrator?.active===true||memberAuthorized||employeeAuthorized||serverAuthorized||tokenAuthorized)return;
...
function resolveLoginCategories(profile,employee,admin){
 const administratorAvailable=false;
