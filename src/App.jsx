...
useEffect(()=>{
  ...
  const unsubscribe=observeAuthState(async u=>{
    ...
    try{
      ...
      const serverRoles=Array.isArray(tokenResult?.claims?.irpaRoles)?tokenResult.claims.irpaRoles:[];
      if(adminDirect?.active===true){
        ...
        setProfile({... (verifiedProfile||verifiedEmployee||adminDirect||{}),uid:u.uid,email:u.email||adminMember?.email||adminEmployee?.email||adminDirect?.email||"",role:assignedRoles[0]||"Administrator",roles:assignedRoles,serverAuthorizedRoles:serverRoles,administratorAvailable:true,authorizationType:"member",enrollmentType:verifiedEmployee&&!verifiedProfile?"employee":"member"});
        return;
      }
