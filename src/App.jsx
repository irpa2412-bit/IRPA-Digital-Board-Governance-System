...
}magic()},[]);const invitationId=params.get("memberInvite");const applicantInductionMode=params.get("applicant")==="1";if(adminGatewayMode)return <AuthScreen/>;if(applicantInductionMode&&inductionMode&&user===undefined)return <Loading/>;if(applicantInductionMode&&inductionMode)return <InductionOrientation/>;if(inductionMode&&user===undefined)return <Loading/>;if(inductionMode)return <InductionOrientation/>;if(user===undefined)return <AuthScreen/>;if(profile===undefined)return <Loading/>;if(!user)return invitationId?<MemberActivationScreen invitationId={invitationId}/>:<AuthScreen/>;
