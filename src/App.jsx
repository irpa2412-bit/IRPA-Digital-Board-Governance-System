  ...
  const unsubscribe=observeAuthState(async u=>{
    ...
    try{
      ...
      if(adminDirect?.active===true){
        ...
        const adminEmployee=adminEmployeeDirect||await getCurrentEmployeeProfile({administratorUid:u.uid,fallbackName:administratorIdentityName}).catch(()=>null);
        ...
        const assignedRoles=[...new Set([...resolveLoginCategories(verifiedProfile,verifiedEmployee,false),...resolveLoginCategories(adminDirect,null,false),"Administrator"].filter(role=>String(role||"").trim()))];
        setEmployee(verifiedEmployee);
        setProfile({... (verifiedProfile||verifiedEmployee||adminDirect||{}),uid:u.uid,email:u.email||adminMember?.email||adminEmployee?.email||adminDirect?.email||"",role:assignedRoles[0]||"Administrator",roles:assignedRoles,administratorAvailable:true,authorizationType:"member",enrollmentType:verifiedEmployee&&!verifiedProfile?"employee":"member"});
        return;
      }
