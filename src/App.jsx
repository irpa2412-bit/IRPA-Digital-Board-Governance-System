...
function AuthorityCapacityGate({categories,onConfirm,onSignOut}){const[selected,setSelected]=useState("");const[feedback,setFeedback]=useState("");const choose=value=>{setSelected(value);setFeedback(value?"Capacity selected provisionally. Confirm to begin navigation with this operating authority.":"")};const confirm=()=>{if(!selected)return;onConfirm(selected)};return <div className="authority-capacity-gate" role="dialog" aria-modal="true" aria-labelledby="authority-capacity-gate-title"><section className="authority-capacity-card"><div className="authority-capacity-kicker">IRPA-DGBS · POST-SIGN-IN CONTROL</div><h1 id="authority-capacity-gate-title">Select Your Operating Authority</h1><p>Your IRPA account has an authorised operating capacity. Identify the capacity you will use for this session <strong>before navigation begins</strong>.</p><div className="authority-capacity-options">{categories.map(category=><button key={category} type="button" className={selected===category?"authority-capacity-option selected":"authority-capacity-option"} onClick={()=>choose(category)} aria-pressed={selected===category}><span>{category}</span><small>{selected===category?"Selected provisionally":"Available authority"}</small></button>)}</div>{feedback&&<div className="authority-capacity-feedback" role="status" aria-live="polite"><strong>PROVISIONAL SELECTION</strong><span>{feedback}</span></div>}<button type="button" className="authority-capacity-confirm" disabled={!selected} onClick={confirm}>Confirm Authority &amp; Enter Dashboard</button><button type="button" className="authority-capacity-signout" onClick={onSignOut}>Sign Out</button><small className="authority-capacity-note">This selection controls the session's visible operating capacity. Existing server-side authorization and role controls remain authoritative.</small></section></div>}
