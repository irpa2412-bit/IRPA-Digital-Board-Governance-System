...
import { downloadDriveBytes, getDownloadURL, ref, uploadBytes } from "./signatureStorage";
...
export async function createSignatureEnvelope({title,documentId,documentReference,documentUrl,signingMode,recipients,fields=[]}){const u=user();if(!title?.trim())throw new Error("Envelope title is required.");if(!documentId)throw new Error("An authorized document must be selected.");if(!documentUrl)throw new Error("The selected document has no PDF file URL. Upload/link the controlled PDF before sending for signature.");if(!recipients?.length)throw new Error("At least one signer is required.");const envelopeId=`IRPA-ENV-${new Date().getFullYear()}-${Date.now().toString().slice(-8)}`;const owner={uid:u.uid,name:u.displayName||u.email||"Document Owner",email:u.email||"",role:"Document Owner",routingOrder:1,status:"Pending"};const supplied=recipients.filter(r=>r?.uid&&r.uid!==u.uid);const orderedRecipients=[owner,...supplied.map((r,i)=>({...r,routingOrder:signingMode==="Sequential"?i+2:r.routingOrder||1}))];const participantUids=orderedRecipients.map(r=>r.uid).filter(Boolean);const payload={envelopeReference:envelopeId,title:title.trim(),documentId,documentReference:documentReference||"",documentUrl,senderUid:u.uid,senderEmail:u.email||"",ownerUid:u.uid,ownerName:owner.name,ownerEmail:owner.email,status:"Draft",signingMode:signingMode||"Sequential",recipients:orderedRecipients,fields,participantUids,currentSignerUid:signingMode==="Sequential"?u.uid:null,createdAt:serverTimestamp(),updatedAt:serverTimestamp()};await setDoc(doc(db,ENVELOPE_COLLECTION,envelopeId),payload);await updateDoc(doc(db,"documents",documentId),{signatureEnvelopeId:envelopeId,signatureReference:envelopeId,signatureStatus:"Draft",signatureOwnerUid:u.uid,updatedAt:serverTimestamp()});await recordEnvelopeEvent(envelopeId,"Envelope Created",{status:"Draft",ownerUid:u.uid,signingOrder:orderedRecipients.map(r=>({uid:r.uid,name:r.name,email:r.email,routingOrder:r.routingOrder}))});return{id:envelopeId,...payload};}
...
export async function signEnvelope(envelope,profile,fieldValues={}){const u=user();if(!profile?.signatureUrl)throw new Error("Your Signature Profile is not configured.");if(!envelope?.documentUrl)throw new Error("This envelope has no controlled PDF URL.");if(!envelope.participantUids?.includes(u.uid))throw new Error("You are not a participant in this signing envelope.");if(envelope.signingMode==="Sequential"&&envelope.currentSignerUid&&envelope.currentSignerUid!==u.uid)throw new Error("This envelope is waiting for another signer.");const source=await fetchBytes(envelope.signedDocumentUrl||envelope.documentUrl);const originalHash=await hashBytes(source);const pdf=await PDFDocument.load(source);const font=await pdf.embedFont(StandardFonts.Helvetica);const signature=await embedImage(pdf,profile.signatureUrl);const initials=profile.initialsUrl?await embedImage(pdf,profile.initialsUrl):null;const signerFields=(envelope.fields||[]).filter(f=>f.signerUid===u.uid);for(const field of signerFields){const page=pdf.getPages()[Math.max(0,Number(field.page||1)-1)];if(!page)continue;const{width,height}=page.getSize();const x=(Number(field.x||10)/100)*width;const y=height-((Number(field.y||80)/100)*height)-(Number(field.height||8)/100)*height;const w=(Number(field.width||24)/100)*width;const h=(Number(field.height||9)/100)*height;if(field.type==="Signature"){page.drawImage(signature,{x,y,width:w,height:h});}else if(field.type==="Initials"&&initials){page.drawImage(initials,{x,y,width:w,height:h});}else{const value=fieldValues[field.fieldId]??(field.type==="Name"?profile.displayName:field.type==="Date"?new Date().toLocaleDateString():fieldValues[field.fieldId]||"");drawField(page,font,field,value);}}const signedBytes=await pdf.save();const finalHash=await hashBytes(signedBytes);const path=`signatureEnvelopes/${envelope.id}/signed-${u.uid}-${finalHash}.pdf`;const storageRef=ref(null,path);await uploadBytes(storageRef,signedBytes,{contentType:"application/pdf",customMetadata:{envelopeId:envelope.id,signerUid:u.uid,originalHash,finalHash}});const signedUrl=await getDownloadURL(storageRef);await setDoc(doc(collection(db,"signatures")),{envelopeId:envelope.id,documentId:envelope.documentId,documentReference:envelope.documentReference,signerUid:u.uid,signerEmail:u.email||"",signerName:profile.displayName,signatureMethod:profile.method||"Upload",signatureSha256:profile.signatureSha256,status:"Signed",signedAt:serverTimestamp(),originalDocumentHash:originalHash,signedDocumentHash:finalHash,immutable:true});
 const signerUids=(envelope.recipients||[]).map(r=>r.uid).filter(Boolean);let completed=false;let nextRecipient=null;if(envelope.signingMode==="Parallel"){const signedSnap=await getDocs(query(collection(db,"signatures"),where("envelopeId","==",envelope.id),where("status","==","Signed")));const signedUids=new Set(signedSnap.docs.map(d=>d.data().signerUid));completed=signerUids.length>0&&signerUids.every(uid=>signedUids.has(uid));}else{const nextIndex=signerUids.findIndex(uid=>uid===u.uid)+1;nextRecipient=envelope.recipients?.[nextIndex];completed=!nextRecipient;}
 const update={status:completed?"Completed":"In Progress",lastSignedByUid:u.uid,lastSignedAt:serverTimestamp(),signedDocumentPath:path,signedDocumentUrl:signedUrl,originalDocumentHash:originalHash,latestDocumentHash:finalHash,updatedAt:serverTimestamp()};if(!completed&&envelope.signingMode!=="Parallel")update.currentSignerUid=nextRecipient.uid;if(completed)update.completedAt=serverTimestamp();else if(envelope.signingMode==="Parallel")update.currentSignerUid=null;await updateDoc(doc(db,ENVELOPE_COLLECTION,envelope.id),update);await recordEnvelopeEvent(envelope.id,"Document Signed",{signerUid:u.uid,signerName:profile.displayName,originalDocumentHash:originalHash,signedDocumentHash:finalHash,status:completed?"Completed":"In Progress",signingMode:envelope.signingMode,requiredSignerCount:signerUids.length});return{...envelope,...update,id:envelope.id};}
