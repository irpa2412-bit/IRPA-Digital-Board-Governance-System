...
export async function getCurrentEmployeeProfile(options={}){const uid=auth.currentUser?.uid;const email=String(auth.currentUser?.email||"").trim().toLowerCase();if(!uid)return null;const records=[];const direct=await getRecord(COLLECTIONS.employees,uid).catch(()=>null);if(direct)records.push(direct);if(email){const snap=await getDocs(query(collection(db,COLLECTIONS.employees),where("email","==",email))).catch(()=>null);if(snap)records.push(...snap.docs.map(x=>({id:x.id,...x.data()})));}if(!records.length&&options?.fallbackName){const name=String(options.fallbackName||"").trim();if(name){const snap=await getDocs(query(collection(db,COLLECTIONS.employees),where("name","==",name))).catch(()=>null);if(snap?.docs?.length===1)records.push({id:snap.docs[0].id,...snap.docs[0].data()});else if(!snap||snap.empty){const all=await getDocs(collection(db,COLLECTIONS.employees)).catch(()=>null);const normalizedName=name.toLowerCase().replace(/\\s+/g," ").trim();const matches=all?.docs?.map(x=>({id:x.id,...x.data()})).filter(x=>String(x.name||"").toLowerCase().replace(/\\s+/g," ").trim()===normalizedName)||[];if(matches.length===1)records.push(matches[0]);}}}return mergeInstitutionalRecords(records);}
