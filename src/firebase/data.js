...
export async function resetEmployeeTrialData(){const uid=await requireActiveAdmin();const recordsDeleted=await deleteCollectionRecords(COLLECTIONS.employees,(data)=>String(data?.recordOrigin||"").toUpperCase()==="TRIAL"||data?.trialData===true||data?.isTrial===true);await setDoc(doc(db,COLLECTIONS.employeeCounters,"employees"),{nextNumber:1,currentNumber:0,updatedAt:serverTimestamp(),source:"administrator-trial-reset"},{merge:true});await writeAudit("ADMIN_RESET_EMPLOYEE_TRIAL_DATA",COLLECTIONS.employees,"TRIAL_RESET",{recordsDeleted,counterReset:true,trialOnly:true,adminUid:uid});return{recordsDeleted,trialOnly:true};}
export async function resetMemberTrialData(){const uid=await requireActiveAdmin();const recordsDeleted=await deleteCollectionRecords(COLLECTIONS.members,(data)=>(String(data?.recordOrigin||"").toUpperCase()==="TRIAL"||data?.trialData===true||data?.isTrial===true)&&!(data?.boardMember===true||data?.role==="Board Member"||data?.boardPosition));await setDoc(doc(db,COLLECTIONS.memberCounters,"members"),{nextNumber:1,currentNumber:0,updatedAt:serverTimestamp(),source:"administrator-trial-reset"},{merge:true});await writeAudit("ADMIN_RESET_MEMBER_TRIAL_DATA",COLLECTIONS.members,"TRIAL_RESET",{recordsDeleted,counterReset:true,boardMembersPreserved:true,trialOnly:true,adminUid:uid});return{recordsDeleted,boardMembersPreserved:true,trialOnly:true};}
