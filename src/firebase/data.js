...
export async function resetSignatureEnvelopeTrialData(){const uid=await requireActiveAdmin();const envelopeSnap=await getDocs(collection(db,"signatureEnvelopes"));const trialEnvelopeIds=envelopeSnap.docs.filter(item=>{const data=item.data()||{};return String(data?.recordOrigin||"").toUpperCase()==="TRIAL"||data?.trialData===true||data?.isTrial===true;}).map(item=>item.id);let envelopesDeleted=0,signaturesDeleted=0,eventsDeleted=0;if(trialEnvelopeIds.length){const signatureSnap=await getDocs(collection(db,COLLECTIONS.signatures));for(const item of signatureSnap.docs){if(trialEnvelopeIds.includes(String(item.data()?.envelopeId||""))){await deleteDoc(item.ref);signaturesDeleted++;}}const eventSnap=await getDocs(collection(db,"signatureEvents"));for(const item of eventSnap.docs){if(trialEnvelopeIds.includes(String(item.data()?.envelopeId||""))){await deleteDoc(item.ref);eventsDeleted++;}}for(const id of trialEnvelopeIds){await deleteDoc(doc(db,"signatureEnvelopes",id));envelopesDeleted++;}}await writeAudit("ADMIN_RESET_SIGNATURE_ENVELOPE_TRIAL_DATA","signatureEnvelopes","TRIAL_RESET",{envelopesDeleted,signaturesDeleted,eventsDeleted,trialOnly:true,adminUid:uid});return{envelopesDeleted,signaturesDeleted,eventsDeleted,trialOnly:true};}
