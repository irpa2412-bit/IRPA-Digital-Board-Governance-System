...
export async function getCurrentMemberProfile(){const uid=auth.currentUser?.uid;const email=String(auth.currentUser?.email||"").trim().toLowerCase();if(!uid)return null;const direct=await getRecord(COLLECTIONS.members,uid);if(direct)return direct;if(!email)return null;const snap=await getDocs(query(collection(db,COLLECTIONS.members),where("email","==",email)));return snap.empty?null:{id:snap.docs[0].id,...snap.docs[0].data()};}
export async function getCurrentEmployeeProfile(){const uid=auth.currentUser?.uid;const email=String(auth.currentUser?.email||"").trim().toLowerCase();if(!uid)return null;const direct=await getRecord(COLLECTIONS.employees,uid);if(direct)return direct;if(!email)return null;const snap=await getDocs(query(collection(db,COLLECTIONS.employees),where("email","==",email)));return snap.empty?null:{id:snap.docs[0].id,...snap.docs[0].data()};}
