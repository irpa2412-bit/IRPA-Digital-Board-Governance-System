...
export async function getRecords(collectionName){const s=await getDocs(collection(db,collectionName));const rows=s.docs.map(x=>({id:x.id,...x.data()}));const toMillis=value=>{if(value?.toMillis)return value.toMillis();if(value instanceof Date)return value.getTime();const n=Date.parse(String(value||""));return Number.isFinite(n)?n:0};return rows.sort((a,b)=>toMillis(b.updatedAt||b.createdAt||b.date)-toMillis(a.updatedAt||a.createdAt||a.date));}
