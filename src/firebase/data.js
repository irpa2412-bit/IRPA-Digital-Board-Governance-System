...
export async function getCurrentInductionContext(){
  ...
  return {
    uid,email,fullName,roles:uniqueRoles,role:uniqueRoles.join(" • "),department,unit,registrationNumber,
...
export async function submitInductionApplication(form,context){
  ...
  await setDoc(doc(db,COLLECTIONS.registrationRequests,uid),payload,{merge:true});
  await setDoc(doc(db,COLLECTIONS.inductionRecords,uid),{
    ...
    requestedDepartment:payload.requestedDepartment||null,
    requestedUnit:payload.requestedUnit||null,
    status:"Pending Administrator LINK",
    inductionStatus:"Submitted",
    roleAssignmentStatus:"Pending",
    applicationId:uid,
    submittedAt,
  ...
  return {alreadyLinked:false,requestId:uid,...payload};
}
