...
function hasInstitutionalRole(record){if(!record)return false;const roles=[record.role,...(Array.isArray(record.roles)?record.roles:[]),...(Array.isArray(record.assignedRoles)?record.assignedRoles:[]),...(Array.isArray(record.selectedRoles)?record.selectedRoles:[]),...(Array.isArray(record.roleAssignments)?record.roleAssignments:[])].flatMap(v=>String(v||"").split(",").map(x=>x.trim()).filter(Boolean));return roles.length>0;}
function mergeInstitutionalRecords(records=[]){const valid=records.filter(Boolean);if(!valid.length)return null;const base=valid.find(hasInstitutionalRole)||valid[0];const roles=[...new Set(valid.flatMap(record=>[record.role,...(Array.isArray(record.roles)?record.roles:[]),...(Array.isArray(record.assignedRoles)?record.assignedRoles:[]),...(Array.isArray(record.selectedRoles)?record.selectedRoles:[]),...(Array.isArray(record.roleAssignments)?record.roleAssignments:[])].flatMap(v=>String(v||"").split(",").map(x=>x.trim()).filter(Boolean)))];const merged={...base};if(roles.length){merged.roles=roles;if(!merged.role)merged.role=roles[0];}return merged;}
