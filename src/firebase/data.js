...
export async function updateEmployee(id,data){if(!id)throw new Error("An employee record ID is required.");const existing=await getRecord(COLLECTIONS.employees,id);if(!existing)throw new Error("The employee record could not be found.");const email=data.email?.trim().toLowerCase();if(email&&email!==String(existing.email||"").toLowerCase()){const q=await getDocs(query(collection(db,COLLECTIONS.employees),where("email","==",email)));if(q.docs.some(x=>x.id!==id))throw new Error(`Another employee is already registered with ${email}.`);}const changes={name:data.name,email,role:data.role,roles:data.roles,assignedRoles:data.assignedRoles,selectedRoles:data.selectedRoles,roleAssignments:data.roleAssignments,department:data.department,unit:data.unit,unitName:data.unitName,jobTitle:data.jobTitle,position:data.position,title:data.title,designation:data.designation,employmentType:data.employmentType,status:data.status,phone:data.phone,startDate:data.startDate,updatedAt:serverTimestamp()};await updateDoc(doc(db,COLLECTIONS.employees,id),changes);await writeAudit("UPDATE_EMPLOYEE_PROFILE",COLLECTIONS.employees,id,{...changes,employeeNumber:existing.employeeNumber});}
...
export async function getCurrentMemberProfile(){const uid=auth.currentUser?.uid;const email=String(auth.currentUser?.email||"").trim().toLowerCase();if(!uid)return null;const records=[];const direct=await getRecord(COLLECTIONS.members,uid).catch(()=>null);if(direct)records.push(direct);const queries=[];if(email)queries.push(query(collection(db,COLLECTIONS.members),where("email","==",email)));if(email)queries.push(query(collection(db,COLLECTIONS.members),where("firebaseAuthEmail","==",email)));queries.push(query(collection(db,COLLECTIONS.members),where("uid","==",uid)));const snaps=await Promise.all(queries.map(q=>getDocs(q).catch(()=>null)));snaps.forEach(s=>s?.docs?.forEach(x=>records.push({id:x.id,...x.data()})));return mergeInstitutionalRecords(records);}
export async function getCurrentEmployeeProfile(options={}){const uid=auth.currentUser?.uid;const email=String(auth.currentUser?.email||"").trim().toLowerCase();if(!uid)return null;const records=[];const collect=(snap)=>{if(snap?.docs?.length)snap.docs.forEach(x=>records.push({id:x.id,...x.data()}));};const direct=await getRecord(COLLECTIONS.employees,uid).catch(()=>null);if(direct)records.push(direct);const identityQueries=[];if(email)identityQueries.push(query(collection(db,COLLECTIONS.employees),where("email","==",email)));if(email)identityQueries.push(query(collection(db,COLLECTIONS.employees),where("firebaseAuthEmail","==",email)));identityQueries.push(query(collection(db,COLLECTIONS.employees),where("uid","==",uid)));const identitySnaps=await Promise.all(identityQueries.map(q=>getDocs(q).catch(()=>null)));identitySnaps.forEach(collect);const administratorUid=String(options?.administratorUid||"").trim();if(!records.length&&administratorUid){for(const field of ["administratorUid","adminUid","linkedAdministratorUid"]){const snap=await getDocs(query(collection(db,COLLECTIONS.employees),where(field,"==",administratorUid))).catch(()=>null);collect(snap);if(records.length)break;}}if(!records.length&&options?.fallbackName){const name=String(options.fallbackName||"").trim();if(name){const snap=await getDocs(query(collection(db,COLLECTIONS.employees),where("name","==",name))).catch(()=>null);if(snap?.docs?.length===1)records.push({id:snap.docs[0].id,...snap.docs[0].data()});else if(!snap||snap.empty){const all=await getDocs(collection(db,COLLECTIONS.employees)).catch(()=>null);const normalizedName=name.toLowerCase().replace(/\\s+/g," ").trim();const matches=all?.docs?.map(x=>({id:x.id,...x.data()})).filter(x=>String(x.name||"").toLowerCase().replace(/\\s+/g," ").trim()===normalizedName)||[];if(matches.length===1)records.push(matches[0]);}}}return mergeInstitutionalRecords(records);}
export async function resolveAuthenticatedLoginContext(){if(!auth.currentUser?.uid)throw new Error("Authentication is required.");const call=httpsCallable(getFunctions(undefined,"us-central1"),"resolveAuthenticatedLoginContext");const result=await call({});return result.data||null;}
...
export async function linkAdministratorToEmployee(employeeId){const uid=auth.currentUser?.uid;if(!uid)throw new Error("Administrator authentication is required.");const admin=await getRecord(COLLECTIONS.adminProfiles,uid);if(!admin?.active)throw new Error("An active Administrator identity is required.");if(!employeeId)throw new Error("An Employee Register record is required.");const employee=await getRecord(COLLECTIONS.employees,employeeId);if(!employee)throw new Error("The selected Employee Register record could not be found.");await updateDoc(doc(db,COLLECTIONS.employees,employeeId),{administratorUid:uid,administratorEmail:String(auth.currentUser?.email||"").trim().toLowerCase(),administratorLinkedAt:serverTimestamp(),updatedAt:serverTimestamp()});await writeAudit("LINK_ADMINISTRATOR_EMPLOYEE_IDENTITY",COLLECTIONS.employees,employeeId,{administratorUid:uid,administratorEmail:String(auth.currentUser?.email||"").trim().toLowerCase(),employeeNumber:employee.employeeNumber||null,employeeRole:employee.role||null});return{employeeId,employeeNumber:employee.employeeNumber||"",role:employee.role||"",administratorUid:uid};}
