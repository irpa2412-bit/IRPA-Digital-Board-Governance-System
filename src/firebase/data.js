...
export async function nextDocumentReference(){const counterRef=doc(db,"systemSettings","documentCounters");return runTransaction(db,async tx=>{const snap=await tx.get(counterRef);const n=Number(snap.data()?.documentReference||0)+1;tx.set(counterRef,{documentReference:n,updatedAt:serverTimestamp()},{merge:true});return "IRPA-DOC-"+new Date().getFullYear()+"-"+String(n).padStart(5,"0")})}
